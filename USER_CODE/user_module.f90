...
!--------------------------------------------------------------------------------------------------!
 MODULE user
    ...
    LOGICAL ::  user_module_enabled = .TRUE.  !<
...
!    INTEGER(iwp) ::  k  !<
    ...
    INTEGER(iwp) ::  fire_start_z
    INTEGER(iwp) ::  fire_end_z
    INTEGER(iwp) ::  fire_start_x
    INTEGER(iwp) ::  fire_end_x
    INTEGER(iwp) ::  fire_start_y
    INTEGER(iwp) ::  fire_end_y
    REAL(wp) ::  fire_tt
    INTEGER(iwp) ::  k 
    INTEGER(iwp) ::  iu ! unit number
    INTEGER(iwp), PARAMETER :: nnx=468, nny=612 ! domain dimension here
    REAL, DIMENSION(nny, nnx):: fire_array
...
!
          ...
          fire_end_y    = 255
          fire_tt      = 993.0_wp
          IF (simulated_time >= fire_start_time) THEN
            IF (coupling_char .EQ. '_N04') THEN
               OPEN(993, FILE='FIRE_DATA', status='old', action='read')
               READ(993, *) ((fire_array(j,i), j=1,nny), i=1,nnx)
               CLOSE(993)
              !-- land use model ---- DQ 08/11/2022
                 DO  m = 1, surf_lsm%ns
                     i = surf_lsm%i(m)
                     j = surf_lsm%j(m)
                     !k = surf_lsm%k(m)
                   DO k = fire_start_z, fire_end_z
                    IF (i>=fire_start_x .AND. i <= fire_end_x .AND. j >= fire_start_y .AND. j <= fire_end_y) THEN
                      IF (fire_array(j,i) >0.0) THEN
                          pt(k, j, i) = fire_array(j,i)
                      ENDIF
                    ENDIF
                   ENDDO
                 ENDDO
