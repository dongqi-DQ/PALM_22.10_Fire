...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE chem_emis_nc_get_time( ntimestamp, timestamp, ncid )
    ...
    USE chem_modules,                                                                              &
        ONLY:  nc_dim_ntime, nc_field_length, nc_var_timestamp

    USE netcdf_data_input_mod,                                                                     &
        ONLY:  get_dimension_length, get_variable
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE chem_emis_nc_get_species( nspecies, emis_species, ncid )
    ...
    USE chem_modules,                                                                              &
        ONLY:  nc_dim_nspecies, nc_field_length, nc_var_species
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE chem_emis_species_match( nspecies, emis_species, ref_species )
    ...
    USE chem_emis_vsrc_mod,                                                                        &   
        ONLY:  default_index   ! default value for no matching index
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE emissions_generic_init_lod2( )
    ...
    USE netcdf_data_input_mod,                                                                     &
        ONLY:  close_input_file, get_attribute,                                                    &
               get_dimension_length, get_variable, open_read_file
    ...
    CALL location_message( 'reading emissions data from '//input_nc_file, 'start' )
    CALL open_read_file  ( input_nc_file, ncid )               ! opens netCDF file
...
!-- allocate space for local volume source emissions
    ...
    CALL location_message( 'reading emissions data from '//input_nc_file, 'finished' )
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE emissions_vsrc_update_lod2( )
    ...
    USE netcdf_data_input_mod,                                                                     &
        ONLY:  close_input_file, get_dimension_length, get_variable, open_read_file
