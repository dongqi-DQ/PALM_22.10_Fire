...
!-- Masked output
    ...
    PUBLIC  dofl_label_x, dofl_label_y, dofl_label_z, dofl_label, dofl_time_count, dofl_unit,      &
            ...
            dots_unit, do2d_unit, do3d_unit, fill_value, id_set_agt, id_set_fl,                    &
 ...
 SUBROUTINE netcdf_define_header( callmode, extend, av )
    ...
    USE control_parameters,                                                                        &
        ONLY:  agent_time_unlimited, air_chemistry, averaging_interval, averaging_interval_pr,     &
               data_output_pr, dcep, domask, dopr_n, dopr_time_count,                              &
               dots_time_count, do2d, do2d_at_begin, do2d_xz_time_count, do3d, do3d_at_begin,      &
               do2d_yz_time_count, dt_data_output_av, dt_do2d_xy, dt_do2d_xz, dt_do2d_yz, dt_do3d, &
               dt_write_agent_data, mask_size, do2d_xy_time_count, do3d_time_count,                &
               domask_time_count, end_time, indoor_model, land_surface, mask_size_l, mask_i,       &
               mask_i_global, mask_j, mask_j_global, mask_k_global, mask_surface, message_string,  &
               ntdim_2d_xy, ntdim_2d_xz, ntdim_2d_yz, ntdim_3d, nz_do3d, ocean_mode, plant_canopy, &
               run_description_header, salsa, section, simulated_time, simulated_time_at_begin,    &
               skip_time_data_output_av, skip_time_do2d_xy, skip_time_do2d_xz, skip_time_do2d_yz,  &
               skip_time_do3d, topography, num_leg, num_var_fl, urban_surface
    ...
    USE spectra_mod,                                                                               &
        ONLY:  averaging_interval_sp, comp_spectra_level, data_output_sp, dosp_time_count,         &
               spectra_direction
...
!--    Check and set accuracy for netCDF output. First set default value.
       ...
       DO  WHILE ( netcdf_precision(i) /= ' ' )
          ...
          SELECT CASE ( var )
             ...
             CASE DEFAULT
                WRITE ( message_string, * ) 'unknown variable in ' // 'initialization_parameters ',&
             ...
             IF ( section(i,1) /= -1 )  THEN
                IF ( zu(section(i,1)) /= netcdf_data(i) )  THEN
                   message_string = 'netCDF file for cross-sections ' //                           &
                                     ...
                                     ' New file is created instead.'
                   CALL message( 'define_netcdf_header', 'PA0251', 0, 1, 0, 6, 0 )
                   extend = .FALSE.
                   RETURN
                ENDIF
             ELSE
...
!-- Set the fill values accordingly to the corresponding output precision.
    IF ( PRESENT( fill ) )  THEN
       IF ( var_type == NF90_REAL4 )  THEN
          nc_stat = NF90_PUT_ATT( ncid, var_id, '_FillValue', REAL( fill_value, KIND = 4 ) )
          CALL netcdf_handle_error( 'netcdf_create_var', 0 )
       ELSE
          nc_stat = NF90_PUT_ATT( ncid, var_id, '_FillValue', REAL( fill_value, KIND = 8 ) )
          CALL netcdf_handle_error( 'netcdf_create_var', 0 )
       ENDIF
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE netcdf_create_global_atts( ncid, data_content, title, error_no )
    ...
    USE control_parameters,                                                                        &
        ONLY:  run_date, run_time, run_zone, runnr, version_string
