...
!-- Pollen model (only Zink is available at the moment).
    SELECT CASE ( TRIM(epol_model) )
       ...
       CASE DEFAULT
          ...
          CALL message( 'init_base_setting', 'PA0812', 0, 1, -1, 6, 0 )
          pollen_model = pollen_model_zink_default
...
!-- TKE scheme.
    SELECT CASE ( TRIM(epol_tke_scheme) )
       ...
       CASE DEFAULT
          message_string = 'Invalid tke scheme selected. Reverting to the default tke scheme.'
          CALL message( 'init_base_setting', 'PA0813', 0, 1, 0, 6, 0 )
          tke_scheme = tke_scheme_default
...
!-- effective referencing to corresponding entries in chem_emis_pollen_data_mod.
    ...
    IF ( num_emis_species == 0 )  THEN
       message_string = 'None of the pollen species names macth with ' //                          &
                        'currently implemented tree pollens. Please check spelling.' 
       CALL message( 'init_species', 'PA0814', 1, 2, 0, 6, 0 )
    ENDIF
...
!--    Read and validate seasonal factors from namelist.
       ...
       IF ( ( emis_species_info(k)%f_season < 0.0_wp )  .OR.                                       &
            ( emis_species_info(k)%f_season > 1.0_wp ) )  THEN
          message_string = 'Seasonal factor can have values only between 0 and 1, ' //             &
                           'reverting to the default value 0.9' 
          CALL message( 'init_species', 'PA0815', 0, 1, 0, 6, 0 )
...
!--    Create reference to data module (use default if species not found).
       ...
       DO  i = pollen_default_offset, pollen_num_total
          IF ( TRIM(emis_species(k)%name_str) /= TRIM(epol_specs_names(i)) )  CYCLE
...
!-- Calculate emission species attributes:
    DO  k = 1,num_emis_species
       ...
       IF ( ( emis_species_info(k)%footprint > 0 )  .OR.  ( emis_species_info(k)%patch > 0 ) )     &
       THEN
          emis_species_info(k)%scale =  dx * dy
       ENDIF

       IF ( emis_species_info(k)%crown > 0 )  THEN
