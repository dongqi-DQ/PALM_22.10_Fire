 ...
 SUBROUTINE time_integration
    ...
    USE control_parameters,                                                                        &
        ONLY:  advected_distance_x, advected_distance_y, air_chemistry, average_count_3d,          &
               ...
               constant_diffusion, coupling_start_time, current_timestep_number,                   &
               debug_output_timestep, debug_string, dcep, disturbance_created,                     &
               disturbance_energy_limit, dist_range, dopr_n, do_sum, dt_3d, dt_averaging_input,    &
               dt_averaging_input_pr, dt_coupling, dt_data_output_av, dt_disturb, dt_do2d_xy,      &
               dt_do2d_xz, dt_do2d_yz, dt_do3d, dt_domask, dt_dopr, dt_dopr_listing,               &
               dt_dots, dt_run_control, end_time, first_call_mas,                                  &
               galilei_transformation, humidity, indoor_model, initializing_actions,               &
               ...
               nr_timesteps_this_run, nudging, ocean_mode, pt_reference, pt_slope_offset,          &
               pt_surface_heating_rate, random_heatflux, salsa, simulated_time,                    &
               simulated_time_chr, skip_time_do2d_xy, skip_time_do2d_xz, skip_time_do2d_yz,        &
               skip_time_do3d, skip_time_domask, skip_time_dopr, skip_time_data_output_av,         &
               sloping_surface, stop_dt, surface_output, syn_turb_gen, terminate_coupled,          &
               terminate_run, timestep_scheme, time_coupling, time_do2d_xy, time_do2d_xz,          &
               time_do2d_yz, time_do3d, time_domask, time_dopr, time_dopr_av, time_dopr_listing,   &
               time_dosp, time_dosp_av, time_dots, time_do_av, time_do_sla,                        &
               time_disturb, time_run_control, time_since_reference_point, timestep_count,         &
...
#if defined( __parallel )
    USE pmc_interface,                                                                             &
        ONLY:  atmosphere_ocean_coupled_run, cpl_id, homogeneous_initialization_child, nested_run, &
               nesting_mode, pmci_adjust_dt_coupling, pmci_atmos_ocean, pmci_boundary_conds,       &
               pmci_datatrans, pmci_synchronize, pmci_ensure_nest_mass_conservation,               &
               pmci_set_swaplevel
#else
    USE pmc_interface,                                                                             &
        ONLY:  atmosphere_ocean_coupled_run, cpl_id
#endif
    ...
    USE arrays_3d,                                                                                 &
        ONLY:  d, dd2zu, ddzu, ddzw,                                                               &
               ...
               v_p,                                                                                &
               w_p,                                                                                &
               kh, km, momentumflux_output_conversion, nc, ni, nr, p, ptdf_x, ptdf_y, qc, qi, qr,  &
               rdf, rdf_sc, rho_air, rho_air_zw, s, tdiss_m, te_m, tpt_m, tu_m, tv_m, tw_m, ug,    &
               u_init, u_stokes_zu, vg, v_init, v_stokes_zu, w, zu

    USE control_parameters,                                                                        &
        ONLY:  tsc

    USE indices,                                                                                   &
        ONLY:  advc_flags_m, advc_flags_s, nyn, nyng, nys, nysg, nz, nzb_max, topo_flags
