...
!-- Arrays for time averages
    ...
    TYPE(surf_type_1d_usm) ::  pt_10cm_av          !< average of theta_10cm (K)
...
!--          Near surface (10cm) temperature for whole surfaces
             IF ( .NOT.  ALLOCATED( pt_10cm_av%val ) )  THEN
                ALLOCATE ( pt_10cm_av%val(1:surf_usm%ns) )
                pt_10cm_av%val = 0.0_wp
             ENDIF
...
!--          Near surface temperature for whole surfaces
             CALL average_surfaces( pt_10cm_av%val, surf_usm%pt_10cm )
...
!
!--          Near surface temperature for whole surfaces
             CALL average_surfaces( pt_10cm_av%val )
...
!--       Near surface temperature for whole surfaces
          IF ( av == 0 )  THEN
             CALL write_surface_data_to_temp_pf( surf_usm%pt_10cm )
          ELSE
             CALL write_surface_data_to_temp_pf( pt_10cm_av%val )
          ENDIF
...
!--    Set-up wall layer discretization
       ...
       DO  k = nzb_wall, nzt_wall-1
          surf_usm%dz_wall_center(k,m) = 0.5_wp * ( surf_usm%dz_wall(k,m) + surf_usm%dz_wall(k+1,m) )
...
!--    Set-up window layer discretization
       ...
       DO  k = nzb_wall, nzt_wall-1
          surf_usm%dz_window_center(k,m) = 0.5_wp * ( surf_usm%dz_window(k,m) + surf_usm%dz_window(k+1,m) )
...
!--       Set ratio of substrate layer thickness, soil-type and LAI
       ...
       DO  k = nzb_wall, nzt_wall-1
          surf_usm%dz_green_center(k,m) = 0.5_wp * ( surf_usm%dz_green(k,m) + surf_usm%dz_green(k+1,m) )
     ...
     IF ( building_pars_f%from_file )  THEN
        DO  m = 1, surf_usm%ns
           IF ( surf_usm%upward(m)  .OR.  surf_usm%downward(m) )  THEN
              ...
              ind_wall_frac    = ind_wall_frac_agfl
              ind_green_frac   = ind_green_frac_r_agfl
              ind_win_frac     = ind_win_frac_agfl
              ind_lai          = ind_lai_r_agfl
              ind_z0           = ind_z0_agfl
              ind_z0qh         = ind_z0qh_agfl
...
!--        but also by inconsistent driver creation.
           IF ( sum_frac /= 0.0_wp )  THEN
              surf_usm%frac(m,:) = surf_usm%frac(m,:) / sum_frac
!
!--        In case all relative fractions are erroneously set to zero, set wall fraction to 1.
           ELSE
              surf_usm%frac(m,ind_veg_wall)  = 1.0_wp
              surf_usm%frac(m,ind_wat_win)   = 0.0_wp
              surf_usm%frac(m,ind_pav_green) = 0.0_wp
           ENDIF
        ENDIF
     ENDDO
!
!--  If fractions were normalized, give an informative message.
     ...
     IF ( relative_fractions_corrected )  THEN
        message_string = 'At some surfaces the relative material fractions do not ' //             &
...
!-- roughness length where necessary.
    DO  m = 1, surf_usm%ns
       IF ( surf_usm%z0(m) >= surf_usm%z_mo(m) )  THEN

          surf_usm%z0(m) = 0.9_wp * surf_usm%z_mo(m)

          WRITE( message_string, * ) 'z0 exceeds surface-layer height at building ' //             &
                                     'surface and is decreased appropriately at grid point ' //    &
                                     '(i,j) = ', surf_usm%i(m)+surf_usm%ioff(m),                   &
                                                 surf_usm%j(m)+surf_usm%joff(m)
          CALL message( 'urban_surface_model_mod', 'PA0503', 0, 0, myid, 6, 0 )
       ENDIF
       IF ( surf_usm%z0h(m) >= surf_usm%z_mo(m) )  THEN

          surf_usm%z0h(m) = 0.9_wp * surf_usm%z_mo(m)
          surf_usm%z0q(m) = 0.9_wp * surf_usm%z_mo(m)

          WRITE( message_string, * ) 'z0h exceeds surface-layer height at vertical building ' //   &
                                     'surface and is decreased appropriately at grid point ' //    &
                                     '(i,j) = ', surf_usm%i(m)+surf_usm%ioff(m),                   &
                                                 surf_usm%j(m)+surf_usm%joff(m)
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE usm_wall_heat_model( during_spinup )
    ...
    REAL(wp)  ::  fac_veg           !< pre-calcualated factor for vegetation fraction
    REAL(wp)  ::  fac_wall          !< pre-calcualated factor for wall fraction
...
!-- Cycle for all surfaces in given direction
    !$OMP PARALLEL DO PRIVATE (fac_veg, fac_wall, kw, m, wtend, wintend, win_absorp) SCHEDULE (STATIC)
    DO  m = 1, surf%ns
...
!
!--    Pre-calculate fraction factors to avoid divisions by zero in case of only window tiles.
       fac_wall = 0.0_wp
       fac_veg  = 0.0_wp
       IF ( surf%frac(m,ind_veg_wall) + surf%frac(m,ind_pav_green) > 0.0_wp )  THEN
          fac_wall = surf%frac(m,ind_veg_wall) /                                                  &
                     ( surf%frac(m,ind_veg_wall) + surf%frac(m,ind_pav_green) )
          fac_veg = surf%frac(m,ind_pav_green) /                                                  &
                     ( surf%frac(m,ind_veg_wall) + surf%frac(m,ind_pav_green) )
       ENDIF
       ...
       wtend(nzb_wall) = ( 1.0_wp / surf%rho_c_wall(nzb_wall,m) ) *                                &
                         ...
                         + fac_wall * surf%wghf_eb(m)                                              &
                         - fac_veg * ( surf%lambda_h_green(nzt_wall,m) * wall_mod(nzt_wall) *      &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE usm_surface_energy_balance( during_spinup )
    ...
    REAL(wp) ::  drho_l_lv                 !< frequently used parameter for green layers
    ...
    REAL(wp) ::  f2                        !< resistance correction term 2
    ...
    REAL(wp) ::  qv1                       !< specific humidity at first grid level
    REAL(wp) ::  q_s                       !< saturation specific humidity
    REAL(wp) ::  rho_lv                    !< frequently used parameter for green layers
    ...
    surf  => surf_usm
    !$OMP PARALLEL DO PRIVATE (m, i, j, k, kk, frac_win, frac_wall, frac_green, lambda_surface,   &
    ...
    !$OMP&             stend_wall, stend_window, stend_green, tend, m_liq_max, i_off, j_off,      &
    !$OMP&             k_off, horizontal ) SCHEDULE (STATIC)
    DO  m = 1, surf%ns
!
...
!--    and does not exceed a maxmium value in case of zero velocities.
...
!--       thus be considered here) f2 = 0 for very dry soils.
          IF ( surf%upward(m) ) THEN
             ...
             DO  kk = nzb_wall, nzt_wall+1
                 m_total = m_total + rootfr%val(nzb_wall,m)                                        &
                                     * MAX( swc%val(nzb_wall,m), wilt%val(nzb_wall,m) )
             ENDDO

             IF ( m_total > wilt%val(nzb_wall,m)  .AND.  m_total < fc%val(nzb_wall,m) )  THEN
                f2 = ( m_total - wilt%val(nzb_wall,m) ) /                                          &
                     ( fc%val(nzb_wall,m) - wilt%val(nzb_wall,m) )
             ELSEIF ( m_total >= fc%val(nzb_wall,m) )  THEN
                f2 = 1.0_wp
             ELSE
                f2 = 1.0E-20_wp
             ENDIF
...
!--          and bare soils.
             ...
             surf%c_liq(m) = MIN( 1.0_wp, ( m_liq_usm%val(m) / m_liq_max )**0.67 )
...
!--          For pavements, only evaporation of liquid water is possible.
          ...
          ELSE
             f_qsws_veg  = rho_lv * ( 1.0_wp - 0.0_wp ) /                                          & !surf%c_liq(m) )
                                ( surf%r_a_green(m) + surf%r_canopy(m) )
             f_qsws_liq  = 0.0_wp ! rho_lv * surf%c_liq(m) / surf%r_a_green(m)
             f_qsws = f_qsws_veg + f_qsws_liq
          ENDIF
!
!--       Calculate derivative of q_s for Taylor series expansion.
          e_s_dt = e_s * ( 17.269_wp / ( t_surf_green%val(m) - 35.86_wp )                          &
...
!!!!!MS: that's not correct, replace by correct formula.
...
!--             runoff as qsws_soil is then not used in the soil model
                IF ( m_liq_usm%val(m) /= m_liq_max )  THEN
...
!--          (conservation of water is violated here)
             m_liq_usm_p%val(m) = MIN( m_liq_usm_p%val(m), m_liq_max )
!
...
!
!--    Write restart data for waste heat from indoor model. Restart data is required in this
!--    case, because the indoor model, where waste heat is computed, is invoked only each hour,
!--    so that waste heat would have zero value until next call of indoor model.
       IF ( indoor_model )  THEN
