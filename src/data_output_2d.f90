                ...
                IF ( av == 0 )  THEN
                   ...
                   ELSE
                      DO  i = nxl, nxr
                         DO  j = nys, nyn
                            DO  k = nzb, nzt
                               local_pf(i,j,k) = MERGE( 0.5_wp * ( u(k,j,i) + u(k,j,i+1) ),           &
                ...
                IF ( av == 0 )  THEN
                   ...
                   ELSE
                      DO  i = nxl, nxr
                         DO  j = nys, nyn
                            DO  k = nzb, nzt
                               local_pf(i,j,k) = MERGE( 0.5_wp * ( v(k,j,i) + v(k,j+1,i) ),           &
                                                        REAL( fill_value, KIND = wp ),                &
                                                        BTEST( topo_flags(k,j,i), 0 ) )
                            ENDDO
                         ENDDO
...
!--                         No interpolation to grid center at k=nzb.
                            local_pf(i,j,nzb) = MERGE( w(nzb,j,i),                                    &
                                                     REAL( fill_value, KIND = wp ),                   &
                                                     BTEST( topo_flags(k,j,i), 0 ) )
                            DO  k = nzb+1, nzt
                               local_pf(i,j,k) = MERGE( 0.5_wp * ( w(k,j,i) + w(k-1,j,i) ),           &
                                                        REAL( fill_value, KIND = wp ),                &
                                                        BTEST( topo_flags(k,j,i), 0 ) )
