 ...
 SUBROUTINE init_pegrid
    ...
    USE indices,                                                                                   &
        ONLY:  nnx, nny, nnz, nx, nxl, nxl_mg, nxlu, nxr, nxr_mg, ny, nyn, nyn_mg, nys, nys_mg,    &
               nysv, nz, nzb, nzt, nzt_mg, topo_flags_1, topo_flags_2, topo_flags_3, topo_flags_4, &
               topo_flags_5, topo_flags_6, topo_flags_7, topo_flags_8, topo_flags_9, topo_flags_10

#if defined( __parallel )
    USE indices,                                                                                   &
        ONLY:  mg_loc_ind, nbgp, nnx_pe, nny_pe, nxl_pe, nxr_pe, nyn_pe, nys_pe
#endif
    ...
    USE transpose_mod,                                                                             &
        ONLY:  nnx_x_max, nxl_y, nxl_z, nxr_x_max, nxr_y, nxr_z, nx_y_max, nyn_x, nyn_x_max,       &
               nyn_z, nys_x, nys_z, ny_z_max, nzb_x, nzb_y, nzt_x, nzt_y, nzt_y_max, nz_x_max
#if defined( __parallel )
    USE transpose_mod,                                                                             &
        ONLY:  nnx_y_max, nny_yd_max, nny_z_max, nnz_x_max, nnz_yd_max, nnz_z_max, nxl_y_pe,       &
               nxl_yd, nxr_y_max, nxr_y_pe, nxr_yd, nyn_yd_max, nyn_z_max, nyn_z_pe, nys_z_pe,     &
               nzb_x_pe, nzb_yd, nzb_yd_pe, nzt_x_max, nzt_x_pe, nzt_yd, nzt_yd_max, nzt_yd_pe,    &
               nz_yd_max
#endif
...
!-- respective comments in main routine).
    IF ( myid_char /= myid_char_prel )  THEN
       ...
       CALL message( 'init_pegrid', 'PA0832', 0, 1, myid, 6, 0 )
    ENDIF
    ...
    IF ( y_shift /= 0 ) THEN
       IF ( bc_lr == 'cyclic' ) THEN
          IF ( TRIM( psolver ) /= 'multigrid' .AND.  TRIM( psolver ) /= 'multigrid_noopt')  THEN
             ...
             CALL message( 'init_pegrid', 'PA0468', 1, 2, 0, 6, 0 )
          ENDIF
...
!--       the recycling method.
          ...
          THEN
             ...
             CALL message( 'init_pegrid', 'PA0467', 1, 2, 0, 6, 0 )
          ENDIF
       ENDIF
    ENDIF
...
!--    This transposition is not neccessary in case of a 1d-decomposition along x
       IF ( psolver(1:7) == 'poisfft'  .OR.  calculate_spectra )  THEN
          IF ( npey /= 1 )  THEN
             IF ( MOD( nz , npex ) /= 0 )  THEN
...
!--    Indices for direct transpositions z --> y (used for calculating spectra)
       IF ( calculate_spectra )  THEN
          IF ( MOD( nz, npey ) /= 0 )  THEN
             WRITE( message_string, * ) 'direct transposition z --> y (needed ',                   &
                                        'for spectra):& nz=', nz, ' is not an ',                   &
                                        'integral multiple of npey=', npey
             CALL message( 'init_pegrid', 'PA0234', 1, 2, 0, 6, 0 )
          ELSE
             nxl_yd = nxl
             nxr_yd = nxr
             nzb_yd = 1 + myidy * ( nz / npey )
             nzt_yd = ( myidy + 1 ) * ( nz / npey )
             sendrecvcount_zyd = nnx * nny * ( nz / npey )
          ENDIF
       ENDIF
...
!
!-- Multigrid solver and 1D-decompositions don't allow non-uniform subdomains
    IF ( psolver(1:9) == 'multigrid'  .OR.  npex == 1  .OR.  npey == 1 )  THEN
...
!
!--    Since PE numbers are integral divisors of total gird point numbers,
!--    the subdomains are uniform.
       non_uniform_subdomain = .FALSE.
!
!--    However, we need to switch back to non-uniform subdomains in case that at least one of
!--    the following conditions is true.
       IF ( MOD(   nz , npex ) /= 0  .OR.  MOD( nx+1 , npey ) /= 0  .OR.                   &
            MOD( ny+1 , npex ) /= 0 )                                                          &
       THEN
          non_uniform_subdomain = .TRUE.
       ENDIF
