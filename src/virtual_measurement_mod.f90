...
!-- Check if the reference coordinate is given.
    IF ( vmea_general%origin_x_pd < 0.0_wp  .OR.  vmea_general%origin_y_pd < 0.0_wp )  THEN
       message_string = "No reference coordinate for the lower-left model domain corner are " //   &
                        "provided, neither in the static input nor in the measurement input file."
       CALL message( 'vm_init', 'PA0534', 1, 2, 0, 6, 0 )
    ENDIF
...
!--          For timeseries output, check if the sampling location lies within the model domain.
             IF ( warn_out_of_domain_locations  .AND.  vmea(l)%timseries )  THEN
                IF ( is < 0  .OR.  is > nx+1  .OR.  js < 0  .OR.  js > ny+1  .OR.                  &
                     zar(t) < 0.0_wp  .OR.  zar(t) >= zw(nzt) )  THEN
                   WRITE( message_string, * ) 'Sampling coordinates of site: ',                    &
                                              TRIM( vmea(l)%site ), ' ',                           &
                                              'do not lie within the PALM domain.'
                   CALL message( 'vm_init', 'PA0848', 0, 1, 0, 6, 0 )
                ENDIF
             ENDIF
!
...
!--          block needs to be moved out of the loop over t.
             IF ( vmea(l)%timseries  .AND.  interpolate_to_location  .AND.  off_ts == 0  .AND.     &
                  off_ts_z == 0 )  THEN
...
!-- Check if DOM reported any error
    ...
    IF ( TRIM( dom_error_message ) /= '' )  THEN
       message_string = 'Error while defining output:' // dom_error_message
       CALL message( 'vm_init_output', 'PA0769', 0, 1, 0, 6, 0 )
    ENDIF
...
!-- Check if DOM reported any error
    dom_error_message = dom_get_error_message()
    IF ( TRIM( dom_error_message ) /= '' )  THEN
       message_string = 'Error while writing output:' // dom_error_message
       CALL message( 'vm_data_output', 'PA0770', 0, 1, 0, 6, 0 )
    ENDIF
...
!--                    Take only values from horizontally-upward facing surfaces.
                       DO  mm = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                          vmea(l)%measured_vars(m,n) = MERGE( surf_lsm%ghf(mm),                    &
                                                              vmea(l)%measured_vars(m,n),          &
                                                              surf_lsm%upward(mm) )
                       ENDDO
                    ENDDO
...
!--                 Take only values from horizontally-upward facing surfaces.
                    DO  mm = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                       vmea(l)%measured_vars_soil(m,n) = MERGE( t_soil%var_2d(k,mm),               &
                                                                vmea(l)%measured_vars_soil(m,n),   &
                                                                surf_lsm%upward(mm) )
                    ENDDO
                    DO  mm = surf_usm%start_index(j,i), surf_usm%end_index(j,i)
                       vmea(l)%measured_vars_soil(m,n) = MERGE( t_wall%val(k,mm),                  &
                                                                vmea(l)%measured_vars_soil(m,n),   &
                                                                surf_lsm%upward(mm) )
                    ENDDO
                 ENDDO
...
!--                    Take only values from horizontally-upward facing surfaces.
                       DO  mm = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                          vmea(l)%measured_vars_soil(m,n) = MERGE( m_soil%var_2d(k,mm),            &
                                                                   vmea(l)%measured_vars_soil(m,n),&
                                                                   surf_lsm%upward(mm) )
                       ENDDO
                    ENDDO
...
!--                    Take only values from horizontally-upward facing surfaces.
                       DO  mm = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                          IF ( surf_lsm%water_surface(m) )                                         &
                             vmea(l)%measured_vars(m,n) = MERGE( t_soil%var_2d(nzt,m),             &
                                                                 vmea(l)%measured_vars(m,n),       &
                                                                 surf_lsm%upward(mm) )
                       ENDDO
