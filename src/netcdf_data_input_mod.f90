...
!
!--    Check for valid setting of origin_lon and origin_lat.
       IF ( ABS( input_file_atts%origin_lon ) > 180.0_wp  .OR.                                     &
            ABS( input_file_atts%origin_lat ) > 90.0_wp )  THEN
          message_string = 'PIDS_STATIC input coordinates "origin_lon" and/or "origin_lat" ' //    &
                           'are outside the allowed range of values.'
          CALL message( 'netcdf_data_input_init', 'PA0830', 1, 2, 0, 6, 0 )
       ENDIF
...
!-- introduced just for 2 variables.
    IF ( soil_type_f%from_file  .AND.  ALLOCATED( soil_type_f%var_3d ) )  THEN
       ...
       DO  k = 0, nz_soil
          ...
          CALL set_lateral_neumann_bc( soil_type_f%var_3d(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( surface_fraction_f%from_file )  THEN
       ...
       DO  k = 0, surface_fraction_f%nf-1
          ...
          CALL set_lateral_neumann_bc( surface_fraction_f%frac(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( building_pars_f%from_file )  THEN
       ...
       DO  k = 0, building_pars_f%np-1
          ...
          CALL set_lateral_neumann_bc( building_pars_f%pars_xy(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( albedo_pars_f%from_file )  THEN
       ...
       DO  k = 0, albedo_pars_f%np-1
          ...
          CALL set_lateral_neumann_bc( albedo_pars_f%pars_xy(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( pavement_pars_f%from_file )  THEN
       ...
       DO  k = 0, pavement_pars_f%np-1
          ...
          CALL set_lateral_neumann_bc( pavement_pars_f%pars_xy(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( vegetation_pars_f%from_file )  THEN
       ...
       DO  k = 0, vegetation_pars_f%np-1
          ...
          CALL set_lateral_neumann_bc( vegetation_pars_f%pars_xy(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( water_pars_f%from_file )  THEN
       ...
       DO  k = 0, water_pars_f%np-1
          ...
          CALL set_lateral_neumann_bc( water_pars_f%pars_xy(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( root_area_density_lsm_f%from_file )  THEN
       ...
       DO  k = 0, root_area_density_lsm_f%nz-1
          ...
          CALL set_lateral_neumann_bc( root_area_density_lsm_f%var(k,:,:) )
       ENDDO
    ENDIF
    ...
    IF ( soil_pars_f%from_file )  THEN
       IF ( soil_pars_f%lod == 1 )  THEN
          ...
          DO  k = 0, soil_pars_f%np-1
             ...
             CALL set_lateral_neumann_bc( soil_pars_f%pars_xy(k,:,:) )
          ENDDO
       ...
       ELSEIF ( soil_pars_f%lod == 2 )  THEN
          ...
          DO  k2 = 0, soil_pars_f%nz-1
             DO  k = 0, soil_pars_f%np-1
                ...
                CALL set_lateral_neumann_bc( soil_pars_f%pars_xyz(k,k2,:,:) )
             ENDDO
          ENDDO
       ENDIF
    ENDIF
    ...
    IF ( pavement_subsurface_pars_f%from_file )  THEN
       ...
       DO  k2 = 0, pavement_subsurface_pars_f%nz-1
          DO  k = 0, pavement_subsurface_pars_f%np-1
             ...
             CALL set_lateral_neumann_bc( pavement_subsurface_pars_f%pars_xyz(k,k2,:,:) )
          ENDDO
       ENDDO
    ENDIF
...
!-- with the PALM grid before reading.
    ...
    IF ( trigger_error_message )  THEN
       message_string = "Initialization with data from dynamic input file - dimension(s): &" //    &
                        TRIM( variable_not_found ) // '& not found in dynamic input file.'
       CALL message( 'netcdf_data_input_mod', 'PA0xxx', 1, 2, 0, 6, 0 )  ! to be named
    ENDIF
...
!-- Moreover, please note, u- and v-grid has 1 grid point less than the dynamic file grid.
    IF ( init_3d%nx-1 /= nx  .OR.  init_3d%nxu-1 /= nx - 1  .OR.                                   &
       ...
       message_string = 'Number of horizontal grid points in dynamic input file does not match ' //&
                        'the number of numeric grid points.'
       CALL message( 'netcdf_data_input_mod', 'PA0543', 1, 2, 0, 6, 0 )
    ENDIF

    IF ( init_3d%nzu /= nz  .OR.  init_3d%nzw /= nz - 1 )  THEN
       message_string = 'Number of vertical grid points in dynamic input file does not match ' //  &
                        'the number of numeric grid points.'
       CALL message( 'netcdf_data_input_mod', 'PA0543', 1, 2, 0, 6, 0 )
    ENDIF
!
...
!-- Note, the check is performed separately for 'z' and 'zw'.
    IF ( ANY( ABS( zu(1:nzt) - init_3d%zu_atmos(1:init_3d%nzu) ) >                                 &
       ...
       DO  WHILE ( k <= nzt )
          ...
          k = k + 1
       ENDDO
       WRITE( message_string, * ) 'Vertical grid in dynamic driver does not match the numeric ',   &
                                  'grid at z(', k, ').'
       CALL message( 'netcdf_data_input_mod', 'PA0543', 1, 2, 0, 6, 0 )
    ENDIF
...
!-- Dynamic input file must be also present if initialization via read_from_file is prescribed.
    IF ( .NOT. input_pids_dynamic  .AND.  ( INDEX( initializing_actions, 'inifor' ) /= 0  .OR.     &
          INDEX( initializing_actions, 'read_from_file' ) /= 0 ) )  THEN
       message_string = 'initializing_actions = "read_from_file" requires dynamic ' //             &
...
!-- Check if requested file exists
    ...
    IF ( .NOT. file_exists )  THEN
       ...
       CALL message( 'open_read_file', 'PA0516', 2, 2, 0, 6, 1 )
    ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE get_variable_2d_real_time_slice( ncid, var_name, var_data, t0, ndata, init_data )
    ...
    REAL(KIND=wp), DIMENSION(:), INTENT(INOUT) ::  var_data  !< variable data (time,position)
