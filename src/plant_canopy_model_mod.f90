...
!--------------------------------------------------------------------------------------------------!
 MODULE plant_canopy_model_mod
    ...
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latentflux           !< plant canopy latent heat flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latentflux_av        !< array for averaging plant canopy latent flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latentrate           !< plant canopy latent heating rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_latentrate_av        !< array for averaging plant canopy latent heating rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensibleflux         !< plant canopy sensible heat flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensibleflux_av      !< array for averaging plant canopy sensible flux
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensiblerate         !< plant canopy heating rate
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::  pcm_sensiblerate_av      !< array for averaging plant canopy sensible heating rate
...
!>
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pcm_calc_transpiration_rate( i, j, k, kk, pcbsw, pcblw, pcbtr, pcblh, pcblhfl )

!
!--  Input parameters
     INTEGER(iwp), INTENT(IN) ::  i, j, k, kk        !< indices of the pc gridbox
     REAL(wp), INTENT(IN)     ::  pcblw              !< lw radiation in gridbox (Wm-3)
     REAL(wp), INTENT(IN)     ::  pcbsw              !< sw radiation in gridbox (Wm-3)
     REAL(wp), INTENT(OUT)    ::  pcblh              !< latent heat from transpiration dT/dt (K/s)
     REAL(wp), INTENT(OUT)    ::  pcblhfl            !< latent heat flux from transpiration (Wm-3)
...
!--  Coefficient for conversion of radiation to grid to radiation to unit leaves surface
     v_lad = 1.0_wp / MAX( lad_s(kk,j,i), 1.0E-10_wp )
!
...
!--  Ngao, Adam and Saudreau (2017) eq. 6 but with water vapour partial pressure
     vpd = MAX( sat_press - q(k,j,i) * hyp(k) / rd_d_rv, 0.0_wp )
!
...
!--  (evaporation rate is limited to non-negative values)
     evaporation_rate = MAX( d_fact * e_eq + ( 1.0_wp - d_fact ) * e_imp, 0.0_wp )
!
!--  Conversion of evaporation rate to q tendency in gridbox
!--  dq/dt = E * LAD * V_g / (rho_air * V_g)
     pcbtr = evaporation_rate * r_d * pt(k,j,i) * exner(k) * lad_s(kk,j,i) / hyp(k)  !-- = dq/dt
!
!--  latent heat from evaporation
     pcblh = pcbtr * lv_d_cp  !-- = - dT/dt
!
!--  latent heat flux from evaporation
     pcblhfl = evaporation_rate * l_v * lad_s(kk,j,i)
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pcm_check_data_output( var, unit, j )
...
!--       dependencies.
          IF ( cthf == 0.0_wp  .AND.  ( .NOT. urban_surface  .AND.  .NOT. land_surface ) )  THEN
             message_string = 'output of "' // TRIM( var ) //                                      &
                              '" requires setting of parameter cthf /= 0.0'
...
!--       dependencies.
       ...
       CASE ( 'pcm_bad', 'pcm_lad' )
          IF ( j == 1 )  THEN
             message_string = 'time averaging of a static quantity ("' // TRIM( var ) //            &
                              '") is not provided'
             CALL message( 'pcm_check_data_output', 'PA0006', 1, 2, 0, 6, 0 )
          ENDIF
          unit = 'm2 m-3'
 ...
 SUBROUTINE pcm_3d_data_averaging( mode, variable )
    ...
    IF ( mode == 'allocate' )  THEN
       ...
       SELECT CASE ( TRIM( variable ) )
          ...
          CASE ( 'pcm_heatrate' )
             IF ( .NOT. ALLOCATED( pcm_sensiblerate_av ) )  THEN
                ALLOCATE( pcm_sensiblerate_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
             ENDIF
             pcm_sensiblerate_av = 0.0_wp
          ...
          CASE ( 'pcm_heatflux' )
             IF ( .NOT. ALLOCATED( pcm_sensibleflux_av ) )  THEN
                ALLOCATE( pcm_sensibleflux_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
             ENDIF
             pcm_sensibleflux_av = 0.0_wp


          CASE ( 'pcm_latentflux' )
             IF ( .NOT. ALLOCATED( pcm_latentflux_av ) )  THEN
                ALLOCATE( pcm_latentflux_av(0:pch_index,nysg:nyng,nxlg:nxrg) )
             ENDIF
             pcm_latentflux_av = 0.0_wp
    ...
    ELSE IF ( mode == 'sum' )  THEN
       ...
       SELECT CASE ( TRIM( variable ) )
          ...
          CASE ( 'pcm_heatflux' )
             IF ( ALLOCATED( pcm_sensibleflux_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_sensibleflux_av(k,j,i) = pcm_sensibleflux_av(k,j,i) +              &
                                                         pcm_sensibleflux(k,j,i)
                         ENDDO
                      ENDIF
                   ENDDO
                ENDDO
          ...
          CASE ( 'pcm_heatrate' )
             IF ( ALLOCATED( pcm_sensiblerate_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_sensiblerate_av(k,j,i) = pcm_sensiblerate_av(k,j,i) +              &
                                                         pcm_sensiblerate(k,j,i)
                         ENDDO
                      ENDIF
                   ENDDO
                ENDDO
          ...
          CASE ( 'pcm_latentflux' )
             IF ( ALLOCATED( pcm_latentflux_av ) ) THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      IF ( pch_index_ji(j,i) /= 0 )  THEN
                         DO  k = 0, pch_index_ji(j,i)
                            pcm_latentflux_av(k,j,i) = pcm_latentflux_av(k,j,i) +                  &
                                                       pcm_latentflux(k,j,i)
                         ENDDO
                      ENDIF
                   ENDDO
                ENDDO
             ...
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensiblerate(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensiblerate_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_heatflux' )
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensibleflux(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_sensibleflux_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
       ...
       CASE ( 'pcm_latentrate' )
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_latentrate(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
       ...
       CASE ( 'pcm_latentflux' )
          IF ( av == 0 )  THEN
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_latentflux(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ELSE
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   DO  k = MAX( 1, nzb_do ), MIN( pch_index_ji(j,i), nzt_do )
                      local_pf(i,j,k) = pcm_latentflux_av(k,j,i)
                   ENDDO
                ENDDO
             ENDDO
          ENDIF
...
!--  Check for the grid. zpc is zu(nzb:nzb+pch_index)
     SELECT CASE ( TRIM( var ) )
        ...
        CASE ( 'pcm_heatrate', 'pcm_heatflux', 'pcm_bad', 'pcm_lad', 'pcm_transpirationrate',      &
               'pcm_latentrate', 'pcm_latentflux' )
           grid_x = 'x'
           grid_y = 'y'
           grid_z = 'zpc'
...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE pcm_init
       ...
       INTEGER(iwp) ::  k_3d!< running index
       ...
       REAL(wp), DIMENSION(:), ALLOCATABLE ::  zlad_tmp !< temporary variable used to check mismatches between
                                                        !< static input data and vertical numeric grid
...
!
!--             Inquire number of vertical vegetation layer and check if levels of provided canopy
!--             match with the vertical numeric grid. Use leaf_area_density_f%nz to store
!--             the number of vertical levels.
                IF ( check_existence( vars_pids, 'zlad' ) )  THEN
                   CALL get_dimension_length( pids_id, leaf_area_density_f%nz, 'zlad' )
                   ALLOCATE( zlad_tmp(0:leaf_area_density_f%nz-1) )
                   CALL get_variable( pids_id, 'zlad', zlad_tmp )
!
!--                Check if canopy information is consistent to numeric grid.
                   IF ( leaf_area_density_f%nz > SIZE( zu ) )  THEN
                      message_string = 'Reading canopy data - too much ' //                        &
                                       'data points along the vertical coordinate.'
                      CALL message( 'pcm_init', 'PA0820', 2, 2, 0, 6, 0 )
                   ENDIF
                   IF ( ANY( ABS( zlad_tmp(0:leaf_area_density_f%nz-1) -                           &
                                  zu(0:leaf_area_density_f%nz-1) ) >                               &
                             0.001_wp * MINVAL( dz(1:number_dz) ) ) )                              &
                   THEN
                      k = 0
                      DO  WHILE ( k <= leaf_area_density_f%nz-1 )
                         IF ( ABS( zlad_tmp(k) - zu(k) ) >                                         &
                              0.001_wp * MINVAL( dz(1:number_dz) ) )  EXIT
                         k = k + 1
                      ENDDO
                      WRITE(message_string,*) 'Reading canopy data - vertical coordinate ' //      &
                                              'do not match numeric grid at zlad(', k, ').'
                      CALL message( 'pcm_init', 'PA0821', 2, 2, 0, 6, 0 )
                   ENDIF
                ELSE
                   message_string = 'Reading canopy data - vertical dimension "zlad" missing.'
                   CALL message( 'pcm_init', 'PA0822', 2, 2, 0, 6, 0 )
                ENDIF
!
!--             Read leaf area density - resolved vegetation.
                IF ( check_existence( vars_pids, 'lad' ) )  THEN
                   ...
                   CALL get_attribute( pids_id, char_fill,                                         &
                                       ...
                                       .FALSE., 'lad' )
!
!--                Inquire number of vertical vegetation layer (just read it again for
!--                sake of consistency.
                   CALL get_dimension_length( pids_id, leaf_area_density_f%nz, 'zlad' )
!
!--                Allocate variable for leaf-area density.
                   ALLOCATE( leaf_area_density_f%var(0:leaf_area_density_f%nz-1,nys:nyn,nxl:nxr) )
...
!--                Inquire number of vertical vegetation layer.
                   CALL get_dimension_length( pids_id, basal_area_density_f%nz, 'zlad' )
!
!--                Allocate variable.
                   ALLOCATE( basal_area_density_f%var(0:basal_area_density_f%nz-1,nys:nyn,nxl:nxr) )
...
!--                Inquire number of vertical soil layers.
                   CALL get_dimension_length( pids_id, root_area_density_lad_f%nz, 'zsoil' )
!
!--                Allocate variable.
                   ALLOCATE( root_area_density_lad_f%var(0:root_area_density_lad_f%nz-1,           &
                                                         nys:nyn,nxl:nxr) )
...
#endif
!
!--    Allocation of arrays pcm_sensiblerate, pcm_transpiration_rate and pcm_latentrate
       ALLOCATE( pcm_sensiblerate(0:pch_index,nysg:nyng,nxlg:nxrg) )
       pcm_sensiblerate = 0.0_wp
       ALLOCATE( pcm_sensibleflux(0:pch_index,nysg:nyng,nxlg:nxrg) )
       pcm_sensibleflux = 0.0_wp

       IF ( humidity )  THEN
          ALLOCATE( pcm_transpiration_rate(0:pch_index,nysg:nyng,nxlg:nxrg) )
          pcm_transpiration_rate = 0.0_wp
          ALLOCATE( pcm_latentrate(0:pch_index,nysg:nyng,nxlg:nxrg) )
          pcm_latentrate = 0.0_wp
          ALLOCATE( pcm_latentflux(0:pch_index,nysg:nyng,nxlg:nxrg) )
...
!--    When using the radiation_interactions, canopy heating (pcm_sensiblerate) and plant canopy
!--    transpiration (pcm_transpiration_rate, pcm_latentrate) are calculated in the RTM after the
...
!--       surface heat flux (shf). Set flux only at upward-facing surfaces.
!--       Start with default surface type
          DO  m = 1, surf_def%ns
             i = surf_def%i(m)
             j = surf_def%j(m)
             IF ( cum_lai_hf(0,j,i) /= 0.0_wp  .AND.  surf_def%upward(m) )                         &
                surf_def%shf(m) = cthf * EXP( -ext_coef * cum_lai_hf(0,j,i) )
          ENDDO
!
!--       Natural surfaces
          DO  m = 1, surf_lsm%ns
             i = surf_lsm%i(m)
             j = surf_lsm%j(m)
             IF ( cum_lai_hf(0,j,i) /= 0.0_wp  .AND.  surf_lsm%upward(m) )                         &
                surf_lsm%shf(m) = cthf * EXP( -ext_coef * cum_lai_hf(0,j,i) )
          ENDDO
!
!--       Urban surfaces
          DO  m = 1, surf_usm%ns
             i = surf_usm%i(m)
             j = surf_usm%j(m)
             IF ( cum_lai_hf(0,j,i) /= 0.0_wp  .AND.  surf_usm%upward(m) )                         &
                surf_usm%shf(m) = cthf * EXP( -ext_coef * cum_lai_hf(0,j,i) )
          ENDDO
!
!
...
!--       Within the different canopy layers the plant-canopy heating rate (pcm_sensiblerate) is
             ...
             DO  j = nysg, nyng
                DO  k = 1, pch_index_ji(j,i)
                   IF ( cum_lai_hf(0,j,i) /= 0.0_wp )  THEN
                      pcm_sensiblerate(k,j,i) = cthf *                                             &
                                                ( EXP( -ext_coef * cum_lai_hf(k,j,i) ) -           &
                                                  EXP( -ext_coef * cum_lai_hf(k-1,j,i) ) ) / dzw(k)
                      k_3d = k + topo_top_ind(j,i,0)
!--       multiply with heat capacity and density
                      pcm_sensibleflux(k,j,i) = pcm_sensiblerate(k,j,i) * c_p *                    &
                                                hyp(k_3d) / ( r_d * pt(k_3d,j,i) * exner(k_3d) )
                   ENDIF
                ENDDO
             ENDDO
          ENDDO
    ...
    SUBROUTINE pcm_rrd_local_ftn( k, nxlf, nxlc, nxl_on_file, nxrf, nxrc, nxr_on_file, nynf, nync, &
       ...
       SELECT CASE ( restart_string(1:length) )
          ...
          CASE ( 'pcm_heatrate_av' )
             ...
             pcm_sensiblerate_av(0:pch_index,nysc-nbgp:nync+nbgp,nxlc-nbgp:nxrc+nbgp) =            &
...
!--    full vertical bounds (bzb:nzt+1) in the restart file and must be re-stored after reading.
       ...
       IF ( array_found )  THEN
          IF ( .NOT. ALLOCATED( pcm_sensiblerate_av ) )  THEN
             ALLOCATE( pcm_sensiblerate_av(nzb:pch_index,nysg:nyng,nxlg:nxrg) )
          ENDIF
          ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
          CALL rrd_mpi_io( 'pcm_heatrate_av', tmp_3d )
          pcm_sensiblerate_av = tmp_3d(nzb:pch_index,:,:)
          DEALLOCATE( tmp_3d )
       ENDIF
...
!--                Determine topography-top index on scalar-grid
                   DO  k = topo_top_ind(j,i,0)+1, topo_top_ind(j,i,0) + pch_index_ji(j,i)
                      ...
                      tend(k,j,i) = tend(k,j,i) + pcm_sensiblerate(kk,j,i)
                   ENDDO
                ENDDO
             ENDDO
...
!
!--                      pcm_transpiration_rate is calculated in radiation model in case of
!--                      plant_canopy_transpiration = .T. to include also the dependecy to the
!--                      radiation in the plant canopy box.
...
!--       full vertical bounds (bzb:nzt+1) in the restart file and must be re-stored before writing.
          IF ( ALLOCATED( pcm_sensiblerate_av ) )  THEN
             ALLOCATE( tmp_3d(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
             tmp_3d(nzb:pch_index,:,:)     = pcm_sensiblerate_av
