...
!-- Check steering parameter values
    ...
    THEN
       message_string = 'illegal nesting boundary conditions: ' // TRIM( nesting_bounds )
       CALL message( 'pmci_init', 'PA0007', 3, 2, 0, 6, 0 )
    ENDIF
...
!-- Check for invalid combinations
    IF ( nested_run  .AND.  atmosphere_ocean_coupled_run )  THEN
       message_string = 'nesting and atmosphere-ocean coupling not allowed together'
       CALL message( 'pmci_modelconfiguration', 'PA0751', 1, 2, 0, 6, 0 )
    ENDIF
#endif
...
!--          match exactly.
             IF ( .NOT. child_matches_parent_along_x )  THEN
                ...
                CALL message( 'pmci_setup_parent', 'PA0012', 3, 2, 0, 6, 0 )
             ENDIF
...
!-- cuboid-array test cases.
    ...
    WRITE( 9, * )
    WRITE( 9, '(A,A,F5.1,A)' ) 'A default value is defined and set for ',                          &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pmci_check_setting_mismatches
    ...
    REAL(wp) ::  dt_coupling_root          !<
    REAL(wp) ::  dt_restart_root           !<
    REAL(wp) ::  dx_root                   !<
    REAL(wp) ::  dy_root                   !<
    REAL(wp) ::  end_time_root             !<
    REAL(wp) ::  restart_time_root         !<
    REAL(wp) ::  time_from_ref_point       !<
    REAL(wp) ::  time_from_ref_point_root  !<
    ...
    IF ( atmosphere_ocean_coupled_run )  THEN
       ...
       IF ( root_model  .AND.  dt_coupling == 9999999.9_wp )  THEN
          message_string = 'dt_coupling is required to be set in atmosphere runtime parameters' // &
                           ' namelist'
          CALL message( 'pmci_check_setting_mismatches', 'PA0003', 1, 2, 0, 6, 0 )
       ENDIF

       IF ( root_model  .AND.  dt_coupling < 0.0_wp )  THEN
          message_string = 'dt_coupling < 0.0 is not allowed'
          CALL message( 'pmci_check_setting_mismatches', 'PA0005', 1, 2, 0, 6, 0 )
       ENDIF
       ...
       IF ( dt_coupling /= dt_coupling_root )  THEN
          ...
          CALL message( 'pmci_check_setting_mismatches', 'PA0004', 0, 1, 0, 6, 0 )
       ...
       IF ( time_from_ref_point /= time_from_ref_point_root )  THEN
          ...
          CALL message( 'pmci_check_setting_mismatches', 'PA0008', 1, 2, 0, 6, 0 )
       ENDIF
       ...
       IF ( dx_root < dx )  THEN
          ...
          CALL message( 'pmci_check_setting_mismatches', 'PA0009', 1, 2, 0, 6, 0 )
       ENDIF
       ...
       IF ( dy_root < dy )  THEN
          ...
          CALL message( 'pmci_check_setting_mismatches', 'PA0010', 1, 2, 0, 6, 0 )
       ENDIF
