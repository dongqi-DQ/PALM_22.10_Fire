...
!-- read from file but topography = 'flat', initialization does not work properly.
    ...
    THEN
       message_string =  'If topography information is provided (via ' //                          &
                         'Netcdf or ASCII input) topography = "read_from_file" is required.'
       CALL message( 'topography_mod', 'PA0437', 1, 2, 0, 6, 0 )
    ENDIF
!
...
!--       Building size has to meet some requirements.
          ...
          THEN
             WRITE( message_string, * ) 'inconsistent building parameters:',                       &
                                        ...
                                        'nx=', nx, 'ny=', ny
             CALL message( 'topography_mod', 'PA0203', 1, 2, 0, 6, 0 )
          ENDIF
...
!--          Street canyon in x direction
          ...
          ELSE
             ...
             CALL message( 'topography_mod', 'PA0204', 1, 2, 0, 6, 0 )
...
!--       Street canyon size has to meet some requirements.
          IF ( canyon_width_x /= 9999999.9_wp )  THEN
             ...
             THEN
                ...
                CALL message( 'topography_mod', 'PA0205', 1, 2, 0, 6, 0 )
             ENDIF
          ELSEIF ( canyon_width_y /= 9999999.9_wp )  THEN
             ...
             THEN
                ...
                CALL message( 'topography_mod', 'PA0206', 1, 2, 0, 6, 0 )
             ENDIF
          ENDIF
          IF ( canyon_width_x /= 9999999.9_wp  .AND.  canyon_width_y /= 9999999.9_wp )  THEN
             ...
             CALL message( 'topography_mod', 'PA0207', 1, 2, 0, 6, 0 )
          ENDIF
...
!--       Check for too large tunnel width. Tunnel axis along y.
          IF ( tunnel_width_x /= 9999999.9_wp )  THEN
             IF ( tunnel_width_x > ( nx + 1 ) * dx )  THEN
                ...
                CALL message( 'topography_mod', 'PA0282', 1, 2, 0, 6, 0 )
             ENDIF
...
!--       Tunnel axis along x.
          IF ( tunnel_width_y /= 9999999.9_wp )  THEN
             IF ( tunnel_width_y > ( ny + 1 ) * dy )  THEN
                ...
                CALL message( 'topography_mod', 'PA0456', 1, 2, 0, 6, 0 )
             ENDIF
...
!--          standard cases 'single_building', 'single_street_canyon', 'tunnel' and 'read_from_file'.
             ...
             CALL message( 'topography_mod', 'PA0239', 1, 2, 0, 6, 0 )
          ELSE
!
!--          The default value is applicable here. Set convention according to topography.
       ...
       THEN
          ...
          CALL message( 'topography_mod', 'PA0240', 1, 2, 0, 6, 0 )
       ENDIF
...
!--    Check whether dimension size in input file matches the model dimensions.
       IF ( dim_static%nx-1 /= nx  .OR.  dim_static%ny-1 /= ny )  THEN
          message_string = 'Static input file: horizontal dimension in x- and/or y-direction ' //  &
                           'do not match the respective model dimension'
          CALL message( 'topography_mod', 'PA0548', 1, 2, 0, 6, 0 )
       ENDIF
!
...
!--    model. The allowed tolerance is 0.1% of the respective model grid spacing.
       IF ( ABS( dim_static%x(1) - dim_static%x(0) - dx ) > 0.001_wp * dx  .OR.                    &
            ABS( dim_static%y(1) - dim_static%y(0) - dy ) > 0.001_wp * dy )  THEN

          message_string = 'Static input file: horizontal grid spacing in x- and/or y' //          &
                           '-direction do not match the respective model grid spacing.'
          CALL message( 'topography_mod', 'PA0549', 1, 2, 0, 6, 0 )
       ENDIF
!
...
!--       Read 2D buildings.
          ...
          ELSE
             ...
             CALL message( 'topography_mod', 'PA0540', 1, 2, 0, 6, 0 )
          ENDIF
       ENDIF
!
...
!--          Check if building information is consistent to numeric grid.
             IF ( buildings_f%nz > SIZE( zu ) )  THEN
                message_string = 'Reading 3D building data - too much data points along the ' //   &
                                 ' vertical coordinate.'
                CALL message( 'topography_mod', 'PA0552', 2, 2, 0, 6, 0 )
             ENDIF
             IF ( ANY( ABS( buildings_f%z(0:buildings_f%nz-1) - zu(0:buildings_f%nz-1) ) >         &
                ...
                DO  WHILE ( k <= buildings_f%nz-1 )
                   ...
                   k = k + 1
                ENDDO
                WRITE( message_string, * ) 'Reading 3D building data - vertical coordinate ' //    &
                                           'do not match numeric grid at z(', k, ').'
                CALL message( 'topography_mod', 'PA0553', 2, 2, 0, 6, 0 )
             ENDIF
          ...
          ELSE
             ...
             CALL message( 'topography_mod', 'PA0541', 1, 2, 0, 6, 0 )
          ENDIF
       ENDIF
!
       ...
       DO  ii = 0, io_blocks-1
          IF ( ii == io_group )  THEN
             ...
             IF ( io_status > 0 )  THEN
                ...
                CALL message( 'topography_mod', 'PA0208', 1, 2, 0, 6, 0 )
             ENDIF
...
!-- inititialization before).
    IF ( input_pids_static )  THEN
       IF ( buildings_f%from_file  .AND.  .NOT. building_id_f%from_file )  THEN
          ...
          CALL message( 'topography_mod', 'PA0542', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
...
!--    Note: This check must be placed here as terrain_height_f is altered later.
       IF ( ANY( terrain_height_f%var == terrain_height_f%fill ) )  THEN
          ...
          CALL message( 'topography_mod', 'PA0550', 2, 2, myid, 6, 0 )
       ENDIF
    ELSE
!
...
!-- Create an informative message if any 1-grid point wide holes were filled.
    IF ( filled )  THEN
       WRITE( message_string, * ) 'Topography was filtered, i.e. holes resolved by only ' //       &
                                  'one grid point were filled during initialization.'
       CALL message( 'topography_mod', 'PA0430', 0, 0, 0, 6, 0 )
    ENDIF
...
!-- Create an informative message if any narrow cavities were filled.
    IF ( num_cavity > 0 )  THEN
       WRITE( message_string, * ) num_cavity, 'narrow cavities of less than ',                     &
                                  filter%num_thresh,                                               &
                                  'horizontal grid points were filled during initialization.'
       CALL message( 'topography_mod', 'PA0831', 0, 0, 0, 6, 0 )
    ENDIF
...
!--    Consistency checks
       IF ( nzb_local_min < 0  .OR.  nzb_local_max  > ( nz + 1 ) )  THEN
          ...
          CALL message( 'topography_mod', 'PA0210', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
!
