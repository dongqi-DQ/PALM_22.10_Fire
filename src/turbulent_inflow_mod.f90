...
!-- Check for correct input of the inflow method.
    ...
    THEN
       ...
       CALL message( 'turbulent_inflow_check_parameters', 'PA0184', 1, 2, 0, 6, 0 )
    ENDIF
...
!--    Turbulent inflow is not allowed in conjunction with mesoscale offline nesting.
       IF ( nesting_offline )  THEN
          ...
          CALL message( 'turbulent_inflow_check_parameters', 'PA0834', 1, 2, 0, 6, 0 )
       ENDIF
!
!--    Turbulent inflow is not allowed in nested child domains.
       IF ( child_domain )  THEN
          ...
          CALL message( 'turbulent_inflow_check_parameters', 'PA0835', 1, 2, 0, 6, 0 )
       ENDIF

       IF ( bc_lr /= 'dirichlet/radiation' )  THEN
          message_string = 'turbulent_inflow_method = "' // TRIM( turbulent_inflow_method ) //     &
                           '"& requires a Dirichlet condition at the inflow boundary.'
          CALL message( 'turbulent_inflow_check_parameters', 'PA0133', 1, 2, 0, 6, 0 )
       ENDIF
!
!--    read-from-file inflow is not allowed in conjunction with a synthetic turbulence generator.
       IF ( syn_turb_gen )  THEN
          message_string = 'turbulent_inflow_method = "' // TRIM( turbulent_inflow_method ) //     &
                           ...
                           'generator.'
          CALL message( 'turbulent_inflow_check_parameters', 'PA0836', 1, 2, 0, 6, 0 )
       ENDIF
...
!--    prerun in the first main run.
       ...
       THEN
          ...
          CALL message( 'turbulent_inflow_check_parameters', 'PA0055', 1, 2, 0, 6, 0 )
       ENDIF
!
!--    Check setting of the recycling plane.
       IF ( recycling_width <= dx  .OR.  recycling_width >= nx * dx )  THEN
          ...
          CALL message( 'turbulent_inflow_check_parameters', 'PA0134', 1, 2, 0, 6, 0 )
       ENDIF
...
!--    read-from-file inflow method requires a dynamic input file.
       IF ( .NOT. input_pids_dynamic )  THEN
          ...
          CALL message( 'turbulent_inflow_check_parameters', 'PA0837', 1, 2, 0, 6, 0 )
       ENDIF
!
...
!--    conservation cannot be guaranteed.
       IF ( turbulent_outflow )  THEN
          ...
          CALL message( 'turbulent_inflow_check_parameters', 'PA0838', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
...
!-- Close input file.
    ...
    IF ( trigger_error_message )  THEN
       ...
       CALL message( 'turbulent_inflow_init', 'PA0839', 1, 2, 0, 6, 0 )
    ENDIF
!
!-- Now, check if all required input variables are available.
    ...
    IF ( trigger_error_message )  THEN
       message_string = 'turbulent_inflow_method = "' // TRIM( turbulent_inflow_method ) //        &
                        '" - variable(s): &' //                                                    &
                        TRIM( variable_not_found ) // '& not found in dynamic input file.'
       CALL message( 'turbulent_inflow_init', 'PA0840', 1, 2, 0, 6, 0 )
    ENDIF
!
!-- After variable checks, further check if dimensions are defined properly.
    IF ( inflow_data%ny-1 /= ny )  THEN
       message_string = 'Number of y-grid points in dynamic input file does not match ' //         &
                        'the number of numeric grid points.'
       CALL message( 'turbulent_inflow_init', 'PA0841', 1, 2, 0, 6, 0 )
    ENDIF
    IF ( inflow_data%nzu /= nz )  THEN
       message_string = 'Number of zu-grid points in dynamic input file does not match ' //        &
                        'the number of numeric grid points.'
       CALL message( 'turbulent_inflow_init', 'PA0842', 1, 2, 0, 6, 0 )
    ENDIF

    IF ( inflow_data%nzw /= nz-1 )  THEN
       message_string = 'Number of zw-grid points in dynamic input file does not match ' //        &
                        'the number of numeric grid points.'
       CALL message( 'turbulent_inflow_init', 'PA0843', 1, 2, 0, 6, 0 )
    ENDIF
!
...
!-- Note, if surface spinup is employed, the spinup time must be subtracted from end_time!
    IF ( inflow_data%time(inflow_data%nt-1) < end_time - spinup_time )  THEN
       WRITE( message_string, *) 'Dynamic input files provides too few time levels for ',          &
                                 'turbulent inflow: &',                                            &
                                 inflow_data%time(inflow_data%nt-1), ' is less than ',             &
                                 end_time - spinup_time
       CALL message( 'turbulent_inflow_init', 'PA0844', 1, 2, 0, 6, 0 )
    ENDIF
!
!-- Check if time starts at zero (plus/minus a small offset).
    IF ( ABS( inflow_data%time(0) ) > 10E-5_wp )  THEN
       message_string = 'Dimension "time_inflow" in dynamic input file must start at 0.0s.'
       CALL message( 'turbulent_inflow_init', 'PA0845', 1, 2, 0, 6, 0 )
    ENDIF
!
!-- Check if time dimension contains any _FillValues.
    IF ( ANY( inflow_data%time == inflow_data%fill_time ) )  THEN
       message_string = 'Dimension "time_inflow" in dynamic input file contains _FillValues.'
       CALL message( 'turbulent_inflow_init', 'PA0846', 1, 2, 0, 6, 0 )
    ENDIF
!
...
!-- is limited to two additional 3D arrays per variable.
    IF ( input_block_size < 2  .OR.  input_block_size > 2 * ( nxr - nxl + 1 ) )  THEN
       ...
       CALL message( 'turbulent_inflow_init', 'PA0847', 1, 2, 0, 6, 0 )
    ENDIF
