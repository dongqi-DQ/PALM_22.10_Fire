...
!-- Import radiation model to obtain input for mean radiant temperature
    USE radiation_model_mod,                                                                       &
        ONLY:  id, ix, iy, iz, mrt_include_sw, mrt_minlevel, mrt_nlevels,                          &
...
!-- Set radiation level if not done by user
    IF ( mrt_minlevel == 0  .AND.  mrt_nlevels == 0 )  THEN
       ...
       mrt_nlevels = 1
    ELSE
       IF ( bio_cell_level < mrt_minlevel  .OR.  bio_cell_level > mrt_minlevel + mrt_nlevels - 1 )  THEN
          WRITE( message_string, * ) 'The combination of the mrt_minlevel = ', mrt_minlevel,      &
                                     ' and mrt_nlevels = ', mrt_nlevels, ' does not include',     &
                                     ' bio_cell_level (gravimetric center of the sample human',   &
                                     ' body 1,1 m).'
          CALL message( 'bio_init', 'PA0816', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
!
