...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE calc_pt_surface
    ...
    !$ACC PARALLEL LOOP PRIVATE(i, j, k, k_off ) &
    !$ACC PRESENT(surf, pt)
    DO  m = 1, surf%ns
       i = surf%i(m)
       j = surf%j(m)
       k = surf%k(m)
       k_off = surf%koff(m)
       surf%pt_surface(m) = MERGE( pt(k+k_off,j,i), surf%pt_surface(m), surf%upward(m) )
    ENDDO
...
!-- Set mixing ratio at surface grid level. ( Only for upward-facing surfs. )
 SUBROUTINE calc_q_surface
    ...
    DO  m = 1, surf%ns
       ...
       k = surf%k(m)
       k_off = surf%koff(m)
       surf%q_surface(m) = MERGE( q(k+k_off,j,i), surf%q_surface(m), surf%upward(m) )
    ENDDO
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE calc_vpt_surface
    ...
    DO  m = 1, surf%ns
       ...
       k = surf%k(m)
       k_off = surf%koff(m)
       surf%vpt_surface(m) = MERGE( vpt(k+k_off,j,i), surf%vpt_surface(m), surf%upward(m) )
    ENDDO
...
!--       Assume saturation for atmosphere coupled to ocean (but not in case of precursor runs).
          ...
          ELSE
             ...
             DO  m = 1, surf%ns
                ...
                k_off = surf%koff(m)
                z_mo = surf%z_mo(m)
                surf%qs(m) = MERGE( kappa * ( q(k,j,i) - q(k+k_off,j,i) )                          &
                                   / ( surf%ln_z_z0q(m) - psi_h( z_mo / surf%ol(m) )               &
                                                        + psi_h( surf%z0q(m) / surf%ol(m) ) ),     &
                                    surf%qs(m),                                                    &
                                    surf%upward(m) )
...
!--          but set to the grid-cell temperature.
             surf%pt_10cm(m) = MERGE(                                                              &
                               surf%pt_surface(m) + surf%ts(m) / kappa                             &
                            * ( LOG( 0.1_wp /  surf%z0h(m) ) - psi_h( 0.1_wp / surf%ol(m) )        &
                                + psi_h( surf%z0h(m) / surf%ol(m) ) ),                             &
                                pt(surf%k(m),surf%j(m),surf%i(m)),                                 &
                                surf%upward(m) )
          ENDDO
