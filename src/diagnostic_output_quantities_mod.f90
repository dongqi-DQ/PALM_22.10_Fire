    ...
    SELECT CASE ( TRIM( var ) )
       ...
       CASE ( 'hr' )
          IF ( neutral )  THEN
             ...
             CALL message( 'diagnostic_output', 'PA0751', 1, 2, 0, 6, 0 )
          ENDIF
          unit = 'K/h'
       ...
       CASE ( 'uu', 'uv', 'uw', 'vu', 'vv', 'vw', 'wu', 'wv', 'ww' )
          unit = 'm2/s2'
       ...
       CASE ( 'ws' )
          IF ( .NOT.  passive_scalar )  THEN
             ...
             CALL message( 'diagnostic_output', 'PA0093', 1, 2, 0, 6, 0 )
          ENDIF
          unit = 'm/s'
...
!--    Treat horizotal cross-section output quantities.
       CASE ( 'kfd*', 'theta_2m*', 'ta_2m*', 'vf25m*', 'vf50m*', 'vf75m*', 'vf100m*', 'vfxxm*',    &
              'vfd25m*', 'vfd50m*', 'vfd75m*', 'vfd100m*', 'vfdxxm*', 'wspeed_10m*' )
!
          unit = 'm3/m/s'
...
!--    s grid surface variables
       CASE ( 'kfd*_xy', 'theta_2m*_xy', 'ta_2m*_xy', 'wspeed_10m*_xy', 'vf25m*_xy', 'vf50m*_xy',  &
              'vf75m*_xy', 'vf100m*_xy', 'vfxxm*_xy', 'vfd25m*_xy', 'vfd50m*_xy', 'vfd75m*_xy',    &
              'vfd100m*_xy', 'vfdxxm*_xy' )
...
!--    u grid
       CASE ( 'uu', 'uu_xy', 'uu_xz', 'uu_yz',                                                     &
              'uv', 'uv_xy', 'uv_xz', 'uv_yz',                                                     &
              'uw', 'uw_xy', 'uw_xz', 'uw_yz' )
!
!--       s grid
...
!
!--    v grid
       CASE ( 'vu', 'vu_xy', 'vu_xz', 'vu_yz',                                                     &
              'vv', 'vv_xy', 'vv_xz', 'vv_yz',                                                     &
              'vw', 'vw_xy', 'vw_xz', 'vw_yz' )
!
!--       s grid
          IF ( interpolate_to_grid_center )  THEN
             grid_x = 'x'
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE doq_output_2d( av, variable, found, grid, mode, local_pf, two_d, nzb_do, nzt_do,       &
    ...
    two_d    = .FALSE.
!
!-- Before copy data onto array local_pf, set it to fill value
    local_pf = fill_value
    ...
    SELECT CASE ( TRIM( variable ) )
       ...
       CASE ( 'div_new_xy', 'div_new_xz', 'div_new_yz' )
           ...
           ELSE
              IF ( .NOT. ALLOCATED( div_new_av ) )  THEN
                 ALLOCATE( div_new_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
                 div_new_av = REAL( fill_value, KIND = wp )
              ENDIF
       ...
       CASE ( 'div_old_xy', 'div_old_xz', 'div_old_yz' )
           ...
           ELSE
              IF ( .NOT. ALLOCATED( div_old_av ) )  THEN
                 ALLOCATE( div_old_av(nzb:nzt+1,nysg:nyng,nxlg:nxrg) )
                 div_old_av = REAL( fill_value, KIND = wp )
              ENDIF
       ...
       CASE ( 'kfd*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( kfd_av ) )  THEN
                ALLOCATE( kfd_av(nysg:nyng,nxlg:nxrg) )
                kfd_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vf25m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vf_25m_av ) )  THEN
                ALLOCATE( vf_25m_av(nysg:nyng,nxlg:nxrg) )
                vf_25m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vf50m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vf_50m_av ) )  THEN
                ALLOCATE( vf_50m_av(nysg:nyng,nxlg:nxrg) )
                vf_50m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vf75m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vf_75m_av ) )  THEN
                ALLOCATE( vf_75m_av(nysg:nyng,nxlg:nxrg) )
                vf_75m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vf100m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vf_100m_av ) )  THEN
                ALLOCATE( vf_100m_av(nysg:nyng,nxlg:nxrg) )
                vf_100m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vfxxm*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vf_xxm_av ) )  THEN
                ...
                vf_xxm_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vfd25m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vfd_25m_av ) )  THEN
                ALLOCATE( vfd_25m_av(nysg:nyng,nxlg:nxrg) )
                vfd_25m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vfd50m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vfd_50m_av ) )  THEN
                ALLOCATE( vfd_50m_av(nysg:nyng,nxlg:nxrg) )
                vfd_50m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vfd75m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vfd_75m_av ) )  THEN
                ALLOCATE( vfd_75m_av(nysg:nyng,nxlg:nxrg) )
                vfd_75m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vfd100m*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vfd_100m_av ) )  THEN
                ALLOCATE( vfd_100m_av(nysg:nyng,nxlg:nxrg) )
                vfd_100m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'vfdxxm*_xy' )
          ...
          ELSE
             IF ( .NOT. ALLOCATED( vfd_xxm_av ) )  THEN
                ALLOCATE( vfd_xxm_av(nysg:nyng,nxlg:nxrg) )
                vfd_xxm_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
                   IF ( surf_def%start_index(j,i) <= surf_def%end_index(j,i)  .OR.                 &
       ...
       CASE ( 'ta_2m*_xy' )        ! 2d-array
          ...
          ELSE
             IF ( .NOT. ALLOCATED( ta_2m_av ) )  THEN
                ALLOCATE( ta_2m_av(nysg:nyng,nxlg:nxrg) )
                ta_2m_av = REAL( fill_value, KIND = wp )
             ENDIF
             DO  i = nxl, nxr
                DO  j = nys, nyn
          ...
          DO  j = nys, nyn
             DO  k = nzb_do, nzt_do
                local_pf(i,j,k) = MERGE( to_be_resorted(k,j,i),                                    &
                                         ...
                                         BTEST( topo_flags(k,j,i), flag_nr ) )
             ENDDO
          ENDDO
       ENDDO
    ENDIF
...
!--          kk defines the index of the first grid level >= 2m.
             pt_2m(j,i) = pt(kk-1,j,i) + ( zw(k-1) + 2.0_wp - zu(kk-1)     ) *                     &
