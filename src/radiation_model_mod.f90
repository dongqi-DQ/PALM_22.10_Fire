...
!-- Configuration parameters (they can be setup in PALM config)
    ...
    CHARACTER(rad_version_len), PARAMETER ::  rad_version = 'RAD v. 5.0'  !< identification of version of binary svf and
                                                                          !< restart files
...
!-- are indexed zenith to nadir top to bottom, so r(1) is the highest ray
    TYPE t_traced_ray
       ...
       REAL(wp) ::  len_horiz  !< length of the whole ray in grid units
...
#if defined( __parallel )
    INTEGER(iwp) ::  niters_surfg   !< number of partional MPI_ALLTOALLV loops
    INTEGER(iwp) ::  nmaxsend_surf  !< maximum number of sended array size per processor
    INTEGER(iwp) ::  n_to_recv      !< receiving dimension in surface exchange
    INTEGER(iwp) ::  n_to_send      !< sending dimension in surface exchange

    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  dsendnow_surf   !< temporary array used in
                                                                !< partional MPI_ALLTOALLV exchange based on bufersize
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  drecvnow_surf   !< temporary array used in
                                                                !< partional MPI_ALLTOALLV exchange based on bufersize
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  int_send_buf    !< id of surface that is send by MPI_ALLTOALLV
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  int_recv_buf    !< id of surface that is received by MPI_ALLTOALLV
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  nsendnow_surf   !< temporary array used in
                                                                !< partional MPI_ALLTOALLV exchange based on bufersize
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  nrecvnow_surf   !< temporary array used in
                                                                !< partional MPI_ALLTOALLV exchange based on bufersize
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  n_source_proc   !< number of sending surfaces between procs
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  n_source_proc_disp  !< displacement in MPI_ALLTOALLV call
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  n_target_proc   !< number of receiving surfaces between procs
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  n_target_proc_disp  !< displacement in MPI_ALLTOALLV call
    INTEGER(iwp), DIMENSION(:), ALLOCATABLE ::  srcsurf_dir     !< auxiliary array for surf
                                                                !< array in MPI surface exchanges

    REAL(wp), DIMENSION(:), ALLOCATABLE ::  recv_bufl  !< recv buffers longwave
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  recv_bufs  !< recv buffers shortwave
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  send_buf   !< sending buffers
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  send_surfinl  !< send surfinl in MPI_ALLTOALLV call
    REAL(wp), DIMENSION(:), ALLOCATABLE ::  recv_surfinl  !< recv surfinl in MPI_ALLTOALLV call
#endif
       ...
       SELECT CASE ( var )
          ...
          CASE ( 'rad_net*', 'rad_lw_in*', 'rad_lw_out*', 'rad_sw_in*', 'rad_sw_out*' )
             ...
             IF ( radiation_only )  THEN
                ...
                CALL message( 'check_parameters', 'PA0788', 1, 2, 0, 6, 0 )
             ENDIF
             unit = 'W/m2'
 ...
 SUBROUTINE radiation_check_data_output_surf( trimvar, unit, av )
    ...
    SELECT CASE ( TRIM( trimvar ) )
       ...
       CASE ( 'rtm_skyvf', 'rtm_skyvft' )
          IF ( av == 1 )  THEN
             message_string = 'averaging is not available for variable ' // TRIM( trimvar )
             CALL message( 'radiation_check_data_output_surf', 'PA0006', 1, 2, 0, 6, 0 )
          ENDIF
          unit = '1'
...
!-- no sense, except radiation-only is employed.
    IF ( .NOT. land_surface  .AND.  .NOT. urban_surface  .AND.  .NOT. radiation_only )  THEN
       message_string = 'Usage of radiation model is only allowed if ' //                          &
                        ...
                        '"radiation_only = .T."'
       CALL message( 'radiation_check_parameters', 'PA0486', 1, 2, 0, 6, 0 )
    ENDIF
!
!-- Further checks for radiation-only mode.
    IF ( ( land_surface  .OR.  urban_surface )  .AND.  radiation_only )  THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0789', 1, 2, 0, 6, 0 )
    ENDIF

    IF ( radiation_only  .AND.  radiation_interactions_on )  THEN
       message_string = 'Usage of radiation model in radiation-only mode in combination with ' //  &
                        '"radiation_interactions_on = .T." &is not allowed.'
       CALL message( 'radiation_check_parameters', 'PA0790', 1, 2, 0, 6, 0 )
    ENDIF
    ...
    IF ( radiation_only  .AND.  radiation_scheme /= 'rrtmg' )  THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0791', 1, 2, 0, 6, 0 )
    ENDIF
    ...
    IF ( radiation_scheme /= 'constant'   .AND.  radiation_scheme /= 'clear-sky'  .AND.            &
       ...
       CALL message( 'radiation_check_parameters', 'PA0405', 1, 2, 0, 6, 0 )
    ELSEIF ( radiation_scheme == 'rrtmg' )  THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0407', 1, 2, 0, 6, 0 )
#endif
#if defined( __rrtmg ) && ! defined( __netcdf )
       message_string = 'radiation_scheme = "rrtmg" requires the use of NetCDF (preprocessor ' //  &
                        'directive -D__netcdf'
       CALL message( 'radiation_check_parameters', 'PA0412', 1, 2, 0, 6, 0 )
#endif
    ELSEIF ( radiation_scheme == 'tenstream' )  THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0774', 1, 2, 0, 6, 0 )
#endif
...
!-- Checks performed only if data is given via namelist only.
    IF ( .NOT. input_pids_static )  THEN
       IF ( albedo_type == 0  .AND.  albedo == 9999999.9_wp  .AND.          &
          ...
          CALL message( 'radiation_check_parameters', 'PA0410', 1, 2, 0, 6, 0 )
       ENDIF
       ...
       IF ( albedo_type == 0  .AND.  ( radiation_scheme == 'rrtmg'  .OR.                           &
          ...
          CALL message( 'radiation_check_parameters', 'PA0411', 1, 2, 0, 6, 0 )
       ENDIF
    ENDIF
!
...
!-- Serial mode does not allow mpi_rma
    ...
    IF ( surface_reflections  .AND.  .NOT. raytrace_mpi_rma )  THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0745', 1, 2, 0, 6, 0 )
    ENDIF
#else
    IF ( raytrace_mpi_rma )  THEN
       message_string = 'raytrace_mpi_rma = .T. not allowed in serial mode'
       CALL message( 'radiation_check_parameters', 'PA0710', 1, 2, 0, 6, 0 )
    ENDIF
#endif
    ...
    IF ( cloud_droplets  .AND.   ( radiation_scheme == 'rrtmg'  .OR.                               &
       ...
       CALL message( 'radiation_check_parameters', 'PA0746', 1, 2, 0, 6, 0 )
    ENDIF
!
!-- Check for dt_radiation
    IF ( dt_radiation <= 0.0 )  THEN
       message_string = 'dt_radiation must be > 0.0'
       CALL message( 'radiation_check_parameters', 'PA0591', 1, 2, 0, 6, 0 )
    ENDIF
!
...
!-- Error message when too small values are set
    IF ( raytrace_discrete_elevs < 4  .OR.  raytrace_discrete_azims < 8 )  THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0735', 1, 2, 0, 6, 0 )
    ENDIF
!-- Warning message when small values are set
    IF ( raytrace_discrete_elevs < 9  .OR.  raytrace_discrete_azims < 18 )  THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0747', 0, 1, 0, 6, 0 )
    ENDIF
...
!-- Error/warning messages for TenStream
    IF ( ts_icollapse /= -1_IINTEGERS  .AND.  ts_icollapse /= 1_IINTEGERS ) THEN
       ...
       CALL message( 'radiation_check_parameters', 'PA0775', 1, 2, 0, 6, 0 )
    ENDIF
#endif
...
!-- init_surface_arrays).
    IF ( radiation_interactions_on )  THEN
       ...
       IF ( radiation_scheme == 'tenstream' ) THEN
          ...
          IF ( biometeorology ) THEN
             ...
             CALL message( 'init_3d_model', 'PA0776', 1, 2, 0, 6, 0 )
          ENDIF
    ...
    ELSEIF ( vertical_surfaces_exist  .OR.  plant_canopy  .OR.  biometeorology )  THEN
       ...
       CALL message( 'radiation_init', 'PA0348', 0, 1, 0, 6, 0 )
    ENDIF
!
...
!-- Warning message when cyclic boundary conditions are set
    IF ( radiation_interactions  .AND.  ( bc_lr_cyc  .OR.  bc_ns_cyc ) )  THEN
       ...
       CALL message( 'radiation_init', 'PA0659', 0, 1, 0, 6, 0 )
    ENDIF
!
...
!--    MS: Why is netcdf directive necessary here?
...
!--    restart run.
       IF ( TRIM( initializing_actions ) /= 'read_restart_data' )  CALL radiation_interaction
...
!-- Check if ts_aldif, ts_aldir, ts_asdif, and ts_asdir are the same
    ...
    IF ( ts_albedo_check  .AND.  .NOT. use_broadband_albedo ) THEN
       ...
       CALL message( 'radiation_tenstream_init', 'PA0781', 1, 2, 0, 6, 0 )
    ENDIF
...
!-- Calculate initial surface albedo for different surfaces
    IF ( .NOT. constant_albedo )  THEN
       ...
       CALL message( 'radiation_tenstream_init', 'PA0777',1, 2, 0, 6, 0 )
    ENDIF
!
!-- Allocate 3d arrays of radiative fluxes and heating rates
    IF ( .NOT. ALLOCATED ( rad_sw_in ) )  THEN
...
!--                         Check if urban surface
                            ...
                            ELSE
                               ...
                               CALL message( 'radiation_tenstream_init', 'PA0778', 1, 2, 0, 6, 0 )
                            ENDIF
                         ENDIF
!
...
!--                            Check if urban surface
                               ...
                               ELSE
                                  WRITE( message_string, * ) 'undefined eastward surface,',        &
                                                             'not urban/land'
                                  CALL message( 'radiation_tenstream_init', 'PA0778', 1, 2, 0, 6,  &
                                                0 )
                               ENDIF
                            ELSE
!
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE find_surface( surface, ii, jj, kk, isurf, vertical_surface  )
    ...
    IF ( .NOT. surface_found )  THEN
       ...
       CALL message( 'radiation_tenstream_init', 'PA0780', 1, 2, 0, 6, 0 )
    ENDIF
...
!--             @todo: Actually such an error message should be located in radiation_check_parameters.
                ...
                CALL message( 'radiation_tenstream_init', 'PA0487', 0, 1, 0, 6, 0 )
             ENDIF
          ENDIF
!
...
!-- Sending out flux, surfoutll
    ...
    DO  i = 1, n_to_send  !< local + surf_start = global index, which is required in svf_in_send_buf
       send_buf(i) = surfoutll(int_send_buf(i) - surf_start_id)
    ENDDO

    CALL rtm_alltoallv( send_buf,  n_source_proc_disp,                                              &
                        recv_bufl, n_target_proc_disp )
#endif
...
#if defined( __parallel )
              temp = svf(1,isvf) * svf(2,isvf) * recv_bufl(isurfsrc)
#else
              temp = svf(1,isvf) * svf(2,isvf) * surfoutll(isurfsrc)
#endif
           ELSE
#if defined( __parallel )
              temp = svf(1,isvf) * recv_bufl(isurfsrc)
#else
...
#if defined( __parallel )
        temp = mrtf(imrtf) * recv_bufl(isurfsrc)
#else
...
#if defined( __parallel )
         !$OMP&   REDUCTION(+:pinswl, pinlwl, pabslwl, pemitlwl, pabs_pc_lwdifl, pcbinlw, send_surfinl) SCHEDULE (STATIC)
#else
...
#if defined( __parallel )
                   send_surfinl(isurfsrc) = send_surfinl(isurfsrc) + pcrad
#else
...
#if defined( __parallel )
                   asrc = facearea(surf(id, int_recv_buf(isurfsrc)))

                   pcbinlw(ipcgb) = pcbinlw(ipcgb)                                                 &
                                    + (csf(1,icsf) * recv_bufl(isurfsrc)                           & ! Absorb from first pass surf emit
                                       - pcrad)                                                    & ! Remove emitted heatflux
                                    * asrc * grid_volume_inverse
                   pabslwl = pabslwl + csf(1,icsf) * recv_bufl(isurfsrc) * asrc
#else
                   asrc = facearea(surf(id, isurfsrc))
                   pcbinlw(ipcgb) = pcbinlw(ipcgb)                                                 &
                                    + (csf(1,icsf) * surfoutll(isurfsrc)                           & ! Absorb from first pass surf emit
                                       - pcrad)                                                    & ! Remove emitted heatflux
                                    * asrc * grid_volume_inverse
...
#if defined( __parallel )
        CALL rtm_alltoallv( send_surfinl, n_target_proc_disp, recv_surfinl, n_source_proc_disp )
        !$OMP PARALLEL DO PRIVATE (i, isurf)
        DO  i = 1, n_to_send
           isurf = int_send_buf(i) - surf_start_id
           surfinl(isurf) = surfinl(isurf) + recv_surfinl(i)
        ENDDO
#endif
     ENDIF
...
!--     Sending out flux, surfoutll
        !$OMP PARALLEL DO PRIVATE (i)
        DO  i = 1, n_to_send  !< local + surf_start = global index
           send_buf(i) = surfoutll(int_send_buf(i) - surf_start_id)
        ENDDO

        CALL rtm_alltoallv( send_buf, n_source_proc_disp, recv_bufl, n_target_proc_disp )
!
!--     Sending out flux, surfoutsl
        !$OMP PARALLEL DO PRIVATE (i)
        DO  i = 1, n_to_send
           send_buf(i) = surfoutsl(int_send_buf(i) - surf_start_id)
        ENDDO

        CALL rtm_alltoallv( send_buf, n_source_proc_disp, recv_bufs, n_target_proc_disp )
...
!--     Reflected radiation
        ...
        DO  isvf = 1, nsvfl
           ...
           isurfsrc = svfsurf(2, isvf)
#if defined( __parallel )
           surfins(isurf) = surfins(isurf) + svf(1,isvf) * svf(2,isvf) * recv_bufs(isurfsrc)
           IF ( plant_lw_interact )  THEN
              surfinl(isurf) = surfinl(isurf) + svf(1,isvf) * svf(2,isvf) * recv_bufl(isurfsrc)
           ELSE
              surfinl(isurf) = surfinl(isurf) + svf(1,isvf) * recv_bufl(isurfsrc)
           ENDIF
#else
...
!--        (future version), then asrc must be stored within `csf'
#if defined( __parallel )
           asrc = facearea(surf(id, int_recv_buf(isurfsrc)))
           temp = csf(1,icsf) * recv_bufs(isurfsrc) * asrc * grid_volume_inverse
#else
           asrc = facearea(surf(id, isurfsrc))
           ...
           IF ( plant_lw_interact )  THEN
#if defined( __parallel )
              temp = csf(1,icsf) * recv_bufl(isurfsrc) * asrc * grid_volume_inverse
#else
...
!--     MRT reflected
        ...
        DO  imrtf = 1, nmrtf
           ...
           isurfsrc = mrtfsurf(2,imrtf)
#if defined( __parallel )
           temp = mrtft(imrtf) * recv_bufs(isurfsrc)
#else
        ...
        IF ( cos_zenith > 0.0_wp )  THEN
           DO  i = nxl, nxr
              DO  j = nys, nyn
                 ...
                 rad_shade_h(j,i) = k
                 swflux_vol(k+1:nz_urban_t,j,i) = rad_sw_in_dir(j,i) * sun_direct_factor * 0.25_wp ! 1/4 = ratio circle_area:sphere_surface
               ENDDO
            ENDDO
         ENDIF
...
!-- Allocate radiation arrays
    ...
    IF ( radiation_volumetric_flux )  THEN
       ...
       ALLOCATE( rad_shade_h(nysg:nyng,nxlg:nxrg) )
    ENDIF
...
!-- Move values from temp to original one
    ALLOCATE( int_recv_buf(n_to_recv) )
    int_recv_buf = recv_buf_temp(1:n_to_recv)
    DEALLOCATE( recv_buf_temp )
!
!-- Calculate srcsurf_dir
    ALLOCATE( srcsurf_dir(1:n_to_recv) )
    DO  i = 1, n_to_recv
        srcsurf_dir(i) = surf( id, int_recv_buf(i) )
    ENDDO
!
!-- Allocate needed arrays
!
!-- Send which surfaces local proc requires from other procs+ displacement
    ALLOCATE( n_target_proc(0:numprocs-1), n_target_proc_disp(0:numprocs) )
!
!-- Recv which surfaces other procs require from local proc + displacement
    ALLOCATE( n_source_proc(0:numprocs-1), n_source_proc_disp(0:numprocs) )
!
...
!-- Can not be parallelized by OMP
    DO  i = 1, n_to_recv
        IF ( ( int_recv_buf(i) > surfstart(proc_id) )  .AND.                                      &
           ( int_recv_buf(i) <= surfstart(proc_id+1) ) )  THEN
!
!--        Surface is between proc_id and proc_id + 1
           n_target_proc(proc_id) = n_target_proc(proc_id) + 1
        ELSE
!
...
!-- Check if all surfaces were assigned to target processor
    IF ( ( SUM( n_target_proc ) /= n_to_recv )  .AND.  debug_output )  THEN
        WRITE( debug_string, * ) 'ERROR IN SUM( n_target_proc ) /= n_to_recv    ',                 &
            'proc_id, numprocs, SUM( n_target_proc ), n_to_recv',                                  &
             proc_id, numprocs, SUM( n_target_proc ), n_to_recv
        CALL debug_message( debug_string, 'info' )
    ENDIF
!
!-- Send how many of target svf #proc requires from others
    CALL MPI_ALLTOALL( n_target_proc, 1, MPI_INTEGER, &
                       n_source_proc, 1, MPI_INTEGER, comm2d, ierr )
    IF ( ierr /= 0  .AND.  debug_output )  THEN
       WRITE( debug_string, * ) 'Error at MPI_ALLTOALL 01:', ierr, size( n_target_proc ),          &
                                size( n_source_proc )
       CALL debug_message( debug_string, 'info' )
    ENDIF
    n_to_send = SUM( n_source_proc )   !< send to others
!
!-- Calculate displacement to AllTOAllV routine
    n_target_proc_disp(0) = 0
    n_source_proc_disp(0) = 0
!
!-- Can not be parallelized by OMP
    DO  i = 1, numprocs
        n_target_proc_disp(i) = n_target_proc_disp(i-1) + n_target_proc(i-1)
        n_source_proc_disp(i) = n_source_proc_disp(i-1) + n_source_proc(i-1)
    ENDDO
!
...
!-- svf_recv_buf store info about local/global index of required surface index from other proc
    ALLOCATE( int_send_buf(n_to_send) )
    ALLOCATE( send_buf(n_to_send) )
    ALLOCATE( recv_bufl(n_to_recv) )
    ALLOCATE( recv_bufs(n_to_recv) )
    ALLOCATE( send_surfinl(n_to_recv) )
    ALLOCATE( recv_surfinl(n_to_send) )
!
...
!-- (e.g. this process may have nothing to send and receive, yet some other still might)
    IF ( bufsize_alltoall <= 0 )  THEN
       niters_surfg = 1
       nmaxsend_surf = HUGE( niters_surfg )
    ELSE
       nmaxsend_surf = bufsize_alltoall
       niters_surf = ( MAXVAL( n_source_proc(:) ) + nmaxsend_surf - 1 ) / nmaxsend_surf
       CALL MPI_ALLREDUCE( niters_surf, niters_surfg, 1, MPI_INTEGER, MPI_MAX, comm2d, ierr )
       IF ( niters_surfg > 1 )  THEN
          WRITE( debug_string, '("The MPI_ALLTOALL call has been split to ",I8," iterations ' //   &
                               'of max. ",I12," records each.")' ) niters_surfg, bufsize_alltoall
          CALL debug_message( debug_string, 'info' )
       ENDIF
    ENDIF
!
!-- Allocation of *now arrays in MPI
    ALLOCATE( dsendnow_surf(numprocs) )
    ALLOCATE( drecvnow_surf(numprocs) )

    drecvnow_surf(:) = n_source_proc_disp(0:numprocs-1)
    dsendnow_surf(:) = n_target_proc_disp(0:numprocs-1)

    ALLOCATE( nsendnow_surf(numprocs) )
    ALLOCATE( nrecvnow_surf(numprocs) )
!
!-- Iterate ALLTOALLV using max-sized buffers
    DO  iter = 1, niters_surfg
       nsendnow_surf(:) = MIN( n_target_proc_disp(1:) - dsendnow_surf(:), nmaxsend_surf )
       nrecvnow_surf(:) = MIN( n_source_proc_disp(1:) - drecvnow_surf(:), nmaxsend_surf )
!
!--    Send integer data
       CALL MPI_ALLTOALLV( int_recv_buf, nsendnow_surf(:), dsendnow_surf(:), MPI_INTEGER,          &
                           int_send_buf, nrecvnow_surf(:), drecvnow_surf(:), MPI_INTEGER,          &
                           comm2d, ierr )
       IF ( ierr /= 0  .AND.  debug_output )  THEN
          WRITE( debug_string, * ) 'Error at MPI_ALLTOALLV int_recv_buf sending:',                 &
                                   ierr, iter, nmaxsend_surf, n_source_proc_disp,                  &
                                   dsendnow_surf, nsendnow_surf, n_target_proc_disp,               &
                                   drecvnow_surf, nrecvnow_surf
          CALL debug_message( debug_string, 'info' )
       ENDIF
!
!--    Shift displacements for next iteration
       dsendnow_surf(:) = dsendnow_surf(:) + nsendnow_surf(:)
       drecvnow_surf(:) = drecvnow_surf(:) + nrecvnow_surf(:)
    ENDDO
#endif
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE rtm_alltoallv( send_float, dsend, recv_float, drecv )
    ...
    drecvnow_surf(:) = drecv(0:numprocs-1)
    dsendnow_surf(:) = dsend(0:numprocs-1)

!
!-- Iterate ALLTOALLv using max-sized buffers
    DO  iter = 1, niters_surfg
       nsendnow_surf(:) = MIN( dsend(1:) - dsendnow_surf(:), nmaxsend_surf )
       nrecvnow_surf(:) = MIN( drecv(1:) - drecvnow_surf(:), nmaxsend_surf )

!
!--    Send floating point data
       CALL MPI_ALLTOALLV( send_float, nsendnow_surf(:), dsendnow_surf(:), MPI_REAL,               &
                           recv_float, nrecvnow_surf(:), drecvnow_surf(:), MPI_REAL,               &
                           comm2d, ierr )

       IF ( ierr /= 0  .AND.  debug_output )  THEN
          WRITE( debug_string, * ) 'Error at MPI_ALLTOALLV rtm_alltoallv:', ierr, iter,            &
                                   nmaxsend_surf, dsend, dsendnow_surf,                            &
                                   nsendnow_surf, drecv, drecvnow_surf, nrecvnow_surf
          CALL debug_message( debug_string, 'info' )
       ENDIF
!
!--    Shift displacements for next iteration
...
!--       checked if they are already obstructed.
          DO WHILE ( s%lowest_mixed_ray > s%lowest_free_ray )
             ...
             IF ( r(s%lowest_mixed_ray)%itarget < 0 )  THEN
                ...
                r(s%lowest_mixed_ray)%len_horiz = ( REAL( topo_full, wp ) + 0.5_wp -               &
                                                    s%origin(1) ) / r(s%lowest_mixed_ray)%zdir
             ENDIF
...
!--                oriented faces
                   DO  k = FLOOR( 0.5_wp + s%origin(1) + r(iray)%zdir * s%lastdist ),              &
                      ...
                      IF ( r(iray)%itarget >= 0 )  THEN
                         ...
                         r(iray)%len_horiz = (REAL(k, wp) - 0.5_wp - s%origin(1)) / r(iray)%zdir
                         EXIT
                      ENDIF
                   ENDDO
                ELSE
             ...
             IF ( r(s%lowest_mixed_ray)%itarget < 0 )  THEN
                 r(s%lowest_mixed_ray)%itarget = gridsurf(lastdir,                                 &
                                                          CEILING( -0.5_wp + s%origin(1) +         &
                                                                   ...
                                                                   s%lastdist ),                   &
                                                          lastcolumn(1), lastcolumn(2))
                 r(s%lowest_mixed_ray)%len_horiz = s%lastdist
             ENDIF
             s%lowest_mixed_ray = s%lowest_mixed_ray - 1
          ENDDO
!
...
!--          Continue raytracing at next process
       ...
       ENDDO
!
!--    Finished forward raytracing. Mark length of remaining rays
       iray = s%lowest_mixed_ray
       DO WHILE ( iray > s%lowest_free_ray )
          IF ( r(iray)%itarget < 0 )  r(iray)%len_horiz = s%lastdist
          iray = iray - 1
       ENDDO
       DO WHILE ( iray > 0 )
          r(iray)%len_horiz = s%lastdist
...
!--    Along what dimension will the next wall crossing be?
       ...
       DO  iray = 1, s%nrays
          IF ( r(iray)%itarget >= 0 )  CYCLE ! only process ray from sky

          IF ( r(iray)%len_horiz > nextdist )  THEN
             IF ( r(iray)%len_horiz < s%lastdist )  THEN
!
!--             Ray starts somewhere in the middle of this column
                CALL process_ray_canopy( iray, .TRUE.,                                             &
                                         s%origin(1) + r(iray)%zdir * r(iray)%len_horiz )
             ELSE
                CALL process_ray_canopy( iray, .TRUE. )
             ENDIF
          ENDIF
...
!-- Allocate global vars which depend on ndsidir
    ...
    WRITE( message_string, * ) 'Precalculated', ndsidir, ' solar positions from ', it, ' timesteps.'
    CALL message( 'radiation_presimulate_solar_pos', 'PA0766', 0, 0, 0, 6, 0 )
...
!--          run which reads svf data is started the following query will be skipped.
             IF ( initializing_actions == 'read_restart_data' )  THEN
                ...
                IF ( numprocs_previous_run /= numprocs )  THEN
                   ...
                   CALL message( 'radiation_read_svf', 'PA0491', 1, 2, 0, 6, 0 )
                ENDIF
...
!--          radiation_read_svf. Therefore these arrays have to be allocated in the following.
...
#if defined( __parallel )
             READ( 88 ) n_to_send, n_to_recv, niters_surfg, nmaxsend_surf
             ALLOCATE( int_send_buf(n_to_send) )
             ALLOCATE( int_recv_buf(n_to_recv) )
             ALLOCATE( n_source_proc_disp(0:numprocs) )
             ALLOCATE( n_target_proc_disp(0:numprocs) )
             ALLOCATE( send_buf(n_to_send) )
             ALLOCATE( recv_bufl(n_to_recv) )
             ALLOCATE( recv_bufs(n_to_recv) )
             ALLOCATE( send_surfinl(n_to_recv) )
             ...
             ALLOCATE( nsendnow_surf(numprocs) )
             ALLOCATE( nrecvnow_surf(numprocs) )
             READ( 88 ) int_send_buf
             READ( 88 ) int_recv_buf
             READ( 88 ) n_source_proc_disp
             READ( 88 ) n_target_proc_disp
#endif
...
!--       Force re-calculation of svfs.
          ...
          WRITE( message_string, '(A,I7,A,I7,A,I7,A,I7,A)' )                                       &
              ...
              ' npey_new = ', npey, '&svf wil be re-calculated'
          CALL message( 'radiation_read_svf', 'PA0517', 0, 0, 0, 6, 0 )
          RETURN
       ENDIF
!
...
!--    Check for errors.
       ...
       IF ( nx_on_file /= nx  .OR.  ny_on_file /= ny )  THEN
          WRITE( message_string, '(A,4(A,I7))' )                                                   &
               ...
               ny
          CALL message( 'radiation_read_svf', 'PA0518', 1, 2, 0, 6, 0 )
       ENDIF
...
!--    Reading core-dependent view-factor data sizes and restore them on the respective variable.
       ...
       CALL rrd_mpi_io_global_array( 'n_to_send', tmp_int )
       n_to_send = tmp_int(myid)

       CALL rrd_mpi_io_global_array( 'n_to_recv', tmp_int )
       n_to_recv = tmp_int(myid)

       CALL rrd_mpi_io_global_array( 'niters_surfg', tmp_int )
       niters_surfg = tmp_int(myid)

       CALL rrd_mpi_io_global_array( 'nmaxsend_surf', tmp_int )
       nmaxsend_surf = tmp_int(myid)
...
!--    Allocate core-dependent buffer arrays.
       ...
       ALLOCATE( nrecvnow_surf(numprocs) )
!
!--    Read int_send_buf. Even though the array is of type integer, it is treated as a real
...
!--    This requires a type conversion using the Fortran function NINT.
       ...
       ALLOCATE( tmp_1d(n_to_send) )
       CALL rrd_mpi_io_surface( 'int_send_buf', tmp_1d )
       int_send_buf = NINT( tmp_1d )
       DEALLOCATE( tmp_1d )
!
!--    Read int_recv_buf.
       ...
       ALLOCATE( tmp_1d(n_to_recv) )
       CALL rrd_mpi_io_surface( 'int_recv_buf', tmp_1d )
       int_recv_buf = NINT( tmp_1d )
       DEALLOCATE( tmp_1d )
!
!--    Read n_source_proc_disp.
       ...
       CALL rrd_mpi_io_surface( 'n_source_proc_disp', tmp_1d )
       n_source_proc_disp = NINT( tmp_1d )
       DEALLOCATE( tmp_1d )
!
!--    Read n_target_proc_disp.
       ...
       ALLOCATE( tmp_1d(0:numprocs) )
       CALL rrd_mpi_io_surface( 'n_target_proc_disp', tmp_1d )
       n_target_proc_disp = NINT( tmp_1d )
       DEALLOCATE( tmp_1d )
#endif
...
!--          Open binary file
             ...
             ENDIF
#if defined( __parallel )
             WRITE( 89 ) n_to_send, n_to_recv, niters_surfg, nmaxsend_surf
             WRITE( 89 ) int_send_buf
             WRITE( 89 ) int_recv_buf
             WRITE( 89 ) n_source_proc_disp
             WRITE( 89 ) n_target_proc_disp
#endif
             IF ( radiation_volumetric_flux ) THEN
                 WRITE( 89 ) shadow_top
                 WRITE( 89 ) skyvf_vol
...
!--    Check, if total number of respective skyview values do not exceed 2**31-1
       DO  i = 1, 7
          IF ( global_sum(i) > max_i4_value )  THEN
             ...
             CALL message( 'radiation_write_svf', 'PA0733', 0, 0, 0, 6, 0 )
          ENDIF
       ENDDO
...
!--    writing to the output file.
       ...
       tmp_int = 0
       tmp_int(myid) = n_to_send
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, tmp_int, SIZE( tmp_int ), MPI_INTEGER, MPI_SUM, comm2d, ierr)
       CALL wrd_mpi_io_global_array( 'n_to_send', tmp_int )

       tmp_int = 0
       tmp_int(myid) = n_to_recv
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, tmp_int, SIZE( tmp_int ), MPI_INTEGER, MPI_SUM, comm2d, ierr)
       CALL wrd_mpi_io_global_array( 'n_to_recv', tmp_int )

       tmp_int = 0
       tmp_int(myid) = niters_surfg
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, tmp_int, SIZE( tmp_int ), MPI_INTEGER, MPI_SUM, comm2d, ierr)
       CALL wrd_mpi_io_global_array( 'niters_surfg', tmp_int )

       tmp_int = 0
       tmp_int(myid) = nmaxsend_surf
       CALL MPI_ALLREDUCE( MPI_IN_PLACE, tmp_int, SIZE( tmp_int ), MPI_INTEGER, MPI_SUM, comm2d, ierr)
       CALL wrd_mpi_io_global_array( 'nmaxsend_surf', tmp_int )
       DEALLOCATE( tmp_int )
!
...
!--    write real-type surface data, convert the integer to real type.
       IF ( ALLOCATED( int_send_buf ) )  THEN
          start_index = n_to_send + 1
          end_index   = n_to_send
...
!--       Write the data. First, compute global start and end indices.
          ...
          IF ( data_to_write )  CALL wrd_mpi_io_surface( 'int_send_buf',                           &
                                                         REAL( int_send_buf, KIND = wp ) )
       ENDIF

       IF ( ALLOCATED( int_recv_buf ) )  THEN
          start_index = n_to_recv + 1
          end_index   = n_to_recv
          start_index(nys,nxl) = 1
          ...
          IF ( data_to_write )  CALL wrd_mpi_io_surface( 'int_recv_buf',                           &
                                                         REAL( int_recv_buf, KIND = wp ) )
       ENDIF

       IF ( ALLOCATED( n_source_proc_disp ) )  THEN
          start_index = SIZE( n_source_proc_disp ) + 1
          end_index   = SIZE( n_source_proc_disp )
          start_index(nys,nxl) = 1
          ...
          IF ( data_to_write )  CALL wrd_mpi_io_surface( 'n_source_proc_disp',                     &
                                                         REAL( n_source_proc_disp, KIND = wp ) )
       ENDIF

       IF ( ALLOCATED( n_target_proc_disp ) )  THEN
          start_index = SIZE( n_target_proc_disp ) + 1
          end_index   = SIZE( n_target_proc_disp )
          start_index(nys,nxl) = 1
          ...
          IF ( data_to_write )  CALL wrd_mpi_io_surface( 'n_target_proc_disp',                     &
                                                         REAL( n_target_proc_disp, KIND = wp ) )
       ENDIF
#endif
!
...
!--              Array of incoming diffuse sw radiation in plant canopy
    ...
    ELSEIF ( mode == 'sum' )  THEN
       ...
       SELECT CASE ( TRIM( var ) )
!
!--       Block of large scale (e.g. RRTMG) radiation output variables only upward faced horizontal
!--       surfaces are considered here
          CASE ( 'rad_net*' )
             IF ( ALLOCATED( rad_net_av ) )  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      DO  m = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                         rad_net_av(j,i) = rad_net_av(j,i) + MERGE( surf_lsm%rad_net(m), 0.0_wp,   &
                                                                    surf_lsm%upward(m) )
                      ENDDO
                      DO  m = surf_usm%start_index(j,i), surf_usm%end_index(j,i)
                         rad_net_av(j,i) = rad_net_av(j,i) + MERGE( surf_usm%rad_net(m), 0.0_wp,   &
                                                                    surf_usm%upward(m) )
                      ENDDO
                   ENDDO
                ENDDO
                   ...
                   DO  j = nys, nyn
                      DO  m = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                         rad_lw_in_xy_av(j,i) = rad_lw_in_xy_av(j,i)                               &
                                                         + MERGE( surf_lsm%rad_lw_in(m), 0.0_wp,   &
                                                                  surf_lsm%upward(m) )
                      ENDDO
                      DO  m = surf_usm%start_index(j,i), surf_usm%end_index(j,i)
                         rad_lw_in_xy_av(j,i) = rad_lw_in_xy_av(j,i)                               &
                                                         + MERGE( surf_usm%rad_lw_in(m), 0.0_wp,   &
                                                                  surf_usm%upward(m) )
                      ENDDO
                   ENDDO
                ENDDO
                   ...
                   DO  j = nys, nyn
                      DO  m = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                         rad_lw_out_xy_av(j,i) = rad_lw_out_xy_av(j,i)                             &
                                                        + MERGE( surf_lsm%rad_lw_out(m), 0.0_wp,   &
                                                                 surf_lsm%upward(m) )
                      ENDDO
                      DO  m = surf_usm%start_index(j,i), surf_usm%end_index(j,i)
                         rad_lw_out_xy_av(j,i) = rad_lw_out_xy_av(j,i)                             &
                                                        + MERGE( surf_usm%rad_lw_out(m), 0.0_wp,   &
                                                                 surf_usm%upward(m) )
                      ENDDO
                   ENDDO
                ENDDO
                   ...
                   DO  j = nys, nyn
                      DO  m = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                         rad_sw_in_xy_av(j,i) = rad_sw_in_xy_av(j,i)                               &
                                                         + MERGE( surf_lsm%rad_sw_in(m), 0.0_wp,   &
                                                                  surf_lsm%upward(m) )
                      ENDDO
                      DO  m = surf_usm%start_index(j,i), surf_usm%end_index(j,i)
                         rad_sw_in_xy_av(j,i) = rad_sw_in_xy_av(j,i)                               &
                                                         + MERGE( surf_usm%rad_sw_in(m), 0.0_wp,   &
                                                                  surf_usm%upward(m) )
                      ENDDO
                   ENDDO
                ENDDO
                   ...
                   DO  j = nys, nyn
                      DO  m = surf_lsm%start_index(j,i), surf_lsm%end_index(j,i)
                         rad_sw_out_xy_av(j,i) = rad_sw_out_xy_av(j,i)                             &
                                                        + MERGE( surf_lsm%rad_sw_out(m), 0.0_wp,   &
                                                                 surf_lsm%upward(m) )
                      ENDDO
                      DO  m = surf_usm%start_index(j,i), surf_usm%end_index(j,i)
                         rad_sw_out_xy_av(j,i) = rad_sw_out_xy_av(j,i)                             &
                                                        + MERGE( surf_usm%rad_sw_out(m), 0.0_wp,   &
                                                                 surf_usm%upward(m) )
                      ENDDO
                   ENDDO
                ENDDO
