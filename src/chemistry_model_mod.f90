...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE chem_check_parameters
    ...
    IF  ( radiation_volumetric_flux .AND. photolysis_shading ) THEN
       ...
       message_string = '3D radiation flux and photolyis shading are turned on'
       CALL message( 'chem_check_parameters', 'PA0523', 0, 0, 0, 6, 0 )

    ELSEIF  ( .NOT. radiation_volumetric_flux .AND.   photolysis_shading ) THEN
    ...
    ELSEIF ( .NOT.  photolysis_shading ) THEN
       message_string = 'Photolyis shading turned off'
       CALL message( 'chem_check_parameters', 'PA0523', 0,0, 0, 6, 0 )
          ...
          IF ( TRIM( spec_name ) == TRIM( chem_species(lsp)%name )  .AND.                          &
             ...
             IF (av == 0)  THEN
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      DO  k = nzb_do, nzt_do
                         local_pf(i,j,k) = MERGE( chem_species(lsp)%conc(k,j,i),                   &
             ...
             ELSE
                DO  i = nxl, nxr
                   DO  j = nys, nyn
                      DO  k = nzb_do, nzt_do
                         local_pf(i,j,k) = MERGE( chem_species(lsp)%conc_av(k,j,i),                &
                                                  REAL( fill_value, KIND = wp ),                   &
                                                  BTEST( topo_flags(k,j,i), 0 ) )
                      ENDDO
                   ENDDO
          ...
          IF ( TRIM( spec_name ) == TRIM( phot_frequen(lsp)%name ) )  THEN
            IF (av == 0)  THEN
               DO  i = nxl, nxr
                  DO  j = nys, nyn
                     DO  k = nzb_do, nzt_do
                         local_pf(i,j,k) = MERGE( phot_frequen(lsp)%freq(k,j,i),                   &
                                                  REAL( fill_value, KIND = wp ),                   &
                                                  BTEST( topo_flags(k,j,i), 0 ) )
                     ENDDO
                  ENDDO
...
!-- possible to initialize only part of the chemistry variables from dynamic input.
    IF ( INDEX( initializing_actions, 'inifor' ) /= 0  .OR.                                        &
...
!--------------------------------------------------------------------------------------------------!
 ...
 SUBROUTINE chem_non_advective_processes()
    ...
    USE chem_modules,         ONLY : emis_pollen
    USE chem_emis_vsrc_mod,   ONLY : chem_emis_vsrc_assign_source
    USE chem_emis_pollen_mod, ONLY : chem_emis_pollen_update_settling
