...
!--          Since the check is performed locally, merge the logical flag from all mpi ranks.
             ...
             IF ( .NOT. rh_valid )  THEN
                WRITE( message_string, *) 'The mixing ratio exceeds the saturation mixing ratio.&',&
                                          'First exceedance after ', time_since_reference_point,   &
                                          'seconds of simulated time.&',                           &
                                          'No further warning will be given. Please check your ',  &
                                          'results carefully.'
                CALL message( 'dynamics_actions', 'PA0829', 0, 1, 0, 6, 0 )
             ENDIF
          ENDIF
...
!--    Dirichlet
       IF ( ibc_pt_b == 0 )  THEN
          ...
          DO  m = 1, bc_hv%ns
             ...
             j = bc_hv%j(m)
             k = bc_hv%k(m)
             pt_p(k+bc_hv%koff(m),j+bc_hv%joff(m),i+bc_hv%ioff(m)) =                               &
                                             pt(k+bc_hv%koff(m),j+bc_hv%joff(m),i+bc_hv%ioff(m))
          ENDDO
!
