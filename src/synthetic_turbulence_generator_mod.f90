...
!--    parametrized. In the latter case, a prescribed boundary-layer depth > 0 is required.
       IF ( neutral )  THEN
          ...
          IF ( .NOT. file_stg_exist  .AND.  boundary_layer_depth <= 0.0_wp )  THEN
             message_string = 'For using parametrized turbulence ' //                              &
                              '(no STG_PROFILES file given) in a neutral simulation,&the ' //      &
                              'prescribed boundary-layer depth must be boundary_layer_depth > 0.'
             CALL message( 'stg_check_parameters', 'PA0780', 1, 2, 0, 6, 0 )
          ELSE
