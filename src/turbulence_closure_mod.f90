...
!--          conditions.
       ...
       ELSEIF ( INDEX( initializing_actions, 'set_constant_profiles' ) /= 0  .OR.                  &
                ...
                INDEX( initializing_actions, 'inifor' ) /= 0  .OR.                                 &
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE diffusion_e( var, var_reference )
    ...
    REAL(wp)     ::  mask_down      !< flag to mask SGS-TKE fluxes at downward facing walls
    REAL(wp)     ::  mask_east      !< flag to mask SGS-TKE fluxes at eastward facing walls
    REAL(wp)     ::  mask_north     !< flag to mask SGS-TKE fluxes at northward facing walls
    REAL(wp)     ::  mask_south     !< flag to mask SGS-TKE fluxes at southward facing walls
    REAL(wp)     ::  mask_west      !< flag to mask SGS-TKE fluxes at westward facing walls
    REAL(wp)     ::  mask_up        !< flag to mask SGS-TKE fluxes at upward facing walls
...
!-- Note, in case of rans_tke_e, the dissipation is already calculated in prognostic_equations
    ...
    !$ACC PRIVATE(i, j, k, mask_down, mask_east, mask_north, mask_south, mask_west, mask_up) &
...
!--          Note, in RANS-mode this masking must not be performed since exlicit Dirichlet
!--          conditions are set at walls.
             ...
             tend(k,j,i) = tend(k,j,i) + (                                                         &
                          ( mask_west  * ( km(k,j,i)+km(k,j,i+1) ) * ( e(k,j,i+1) - e(k,j,i) )     &
                          - mask_east  * ( km(k,j,i)+km(k,j,i-1) ) * ( e(k,j,i)   - e(k,j,i-1) )   &
                          ) * ddx2                                                                 &
                        + ( mask_south * ( km(k,j,i)+km(k,j+1,i) ) * ( e(k,j+1,i) - e(k,j,i) )     &
                          - mask_north * ( km(k,j,i)+km(k,j-1,i) ) * ( e(k,j,i)   - e(k,j-1,i) )   &
                          ) * ddy2                                                                 &
                        + ( mask_down  * ( km(k,j,i)+km(k+1,j,i) ) * ( e(k+1,j,i) - e(k,j,i) )     &
                                                                   * ddzu(k+1) * rho_air_zw(k)     &
                          - mask_up    * ( km(k,j,i)+km(k-1,j,i) ) * ( e(k,j,i)   - e(k-1,j,i) )   &
...
!--    masking must not be performed since exlicit Dirichlet conditions are set at walls.
       mask_down  = MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(k+1,j,i), 0 )  .AND.  .NOT. rans_mode )
       mask_east  = MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(k,j,i-1), 0 )  .AND.  .NOT. rans_mode )
       ...
       mask_west  = MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(k,j,i+1), 0 )  .AND.  .NOT. rans_mode )
       mask_up    = MERGE( 1.0_wp, 0.0_wp, BTEST( topo_flags(k-1,j,i), 0 )  .AND.  .NOT. rans_mode )
