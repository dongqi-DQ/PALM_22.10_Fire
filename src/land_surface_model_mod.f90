...
!--------------------------------------------------------------------------------------------------!
 MODULE land_surface_model_mod
    ...
    USE arrays_3d,                                                                                 &
        ONLY:  d_exner, exner, hyp, hyrho, pt, prr, q, ql, vpt, u, v, w

    USE basic_constants_and_equations_mod,                                                         &
        ONLY:  c_p, g, lv_d_cp, l_v, magnus, rho_l, r_d, rd_d_rv
    ...
    USE control_parameters,                                                                        &
        ONLY:  cloud_droplets, coupling_char,                                                      &
               ...
               debug_output,                                                                       &
               debug_output_timestep, dcep, debug_string, dt_3d, humidity,                         &
               intermediate_timestep_count, initializing_actions, intermediate_timestep_count_max, &
               land_surface,                                                                       &
               read_spinup_data,                                                                   &
               restart_data_format_output, rho_surface,                                            &
...
!   otherwise -> no generic variable and user setting is optional
    REAL(wp) :: alpha_vangenuchten = 9999999.9_wp,            & !< NAMELIST alpha_vg
                ...
                dt_lsm = HUGE( 1.0_wp ),                      & !< maximum allowed timestep of the land-surface model
...
!-- Give informative message about deprecated setting surface_type.
    IF ( TRIM( surface_type ) == 'netcdf' )  THEN
       ...
       CALL message( 'lsm_check_parameters', 'PA0819', 0, 0, 0, 0, 0 )
    ENDIF
!
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lsm_surface_energy_balance
    ...
    !$OMP&                     coef_1, coef_2, tend, i_off, j_off, k_off ) SCHEDULE (STATIC)
    DO  m = 1, surf%ns
!
...
!--          Check if at least one type is set.
             IF ( .NOT. surf_lsm%vegetation_surface(m)  .AND.                                      &
                ...
                WRITE( message_string, * ) 'Surface element ' //                                   &
                                           ' at i, j = ',  i, j,                                   &
...
!--    Further, check also for negative soil-moisture values.
       ...
       IF ( flag_exceed_sat )  THEN
          WRITE( message_string, * ) 'soil moisture is higher than its saturation value ' //       &
                                     'at one/more locations. Soil moisture is limited to ' //      &
                                     'its saturation value to maintain stability.'

          CALL message( 'lsm_init', 'PA0458', 0, 1, 0, 6, 0 )
       ENDIF
       ...
       IF ( flag_negative_value )  THEN
          ...
          CALL message( 'lsm_init', 'PA0828', 1, 2, 0, 6, 0 )
       ENDIF
!
...
!-- Note, to avoid that the job-protocoll is messed-up, this message is only given once.
    ...
    IF ( flag_exceed_z0 )  THEN
       WRITE( message_string, * ) 'z0 exceeds surface-layer height at natural surface(s) and ' //  &
    ...
    IF ( flag_exceed_z0h )  THEN
       WRITE( message_string, * ) 'z0h exceeds surface-layer height at natural surface(s) and ' // &
                                  'is decreased appropriately.'
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE lsm_soil_model( calc_soil_moisture )
    ...
    REAL(wp), DIMENSION(nzb_soil:nzt_soil) ::  lambda_temp  !< temp. lambda
    REAL(wp), DIMENSION(nzb_soil:nzt_soil) ::  max_ts       !< maximum allowed timestep according diffusion criterion
    REAL(wp), DIMENSION(nzb_soil:nzt_soil) ::  tend         !< tendency
...
!
!--       Calculate soil heat conductivity (lambda_h) at the _layer level using the weighted average.
!--       For pavement surface, the true pavement depth is considered.
          DO  k = nzb_soil, nzt_soil-1
             surf%lambda_h(k,m) = (lambda_temp(k) * dz_soil(k) +  lambda_temp(k+1) * dz_soil(k+1) ) &
...
!--             interpolation. To do: replace this with ECMWF-IFS Eq. 8.81.
                DO  k = nzb_soil, nzt_soil-1
                   surf%lambda_w(k,m) = (lambda_temp(k) * dz_soil(k) + lambda_temp(k+1) * dz_soil(k+1) ) &
                                        * 0.5_wp * ddz_soil_center(k)
                   surf%gamma_w(k,m)  = (gamma_temp(k) * dz_soil(k) + gamma_temp(k+1) * dz_soil(k+1) )   &
                                  * 0.5_wp * ddz_soil_center(k)
                ENDDO
!
!
...
!--       the last substep.
          IF ( intermediate_timestep_count == intermediate_timestep_count_max )  THEN
             DO  k = nzb_soil, nzt_soil
                max_ts(k) = surf%rho_c_total(k,m) / surf%lambda_h(k,m) * ( dz_soil(k) )**2
             ENDDO
             surf_lsm%dt_max(m) = MINVAL( max_ts(:) )
          ENDIF

       ENDIF ! not water surface
...
!-- Consider a pre-factor (1/8) for the diffusion criterion.
    IF ( ALLOCATED( surf_lsm%dt_max ) )                                                            &
       dt_lsm = MINVAL( surf_lsm%dt_max ) * 0.125_wp
#if defined( __parallel )
    IF ( collective_wait )  CALL MPI_BARRIER( comm2d, ierr )
...
!--       zero. Note, this is only applied for upward-facing surfaces.
          ...
          IF ( surf_lsm%z0(m) > 0.1_wp * surf_lsm%z_mo(m) )  THEN
             surf_lsm%z0(m) = 0.1_wp * surf_lsm%z_mo(m)
             flag_exceed_z0   = .TRUE.
          ENDIF

          IF ( surf_lsm%z0h(m) >= 0.1_wp * surf_lsm%z_mo(m) )  THEN
             surf_lsm%z0h(m) = 0.1_wp * surf_lsm%z_mo(m)
             flag_exceed_z0h   = .TRUE.
          ENDIF

          IF ( surf_lsm%z0q(m) >= 0.1_wp * surf_lsm%z_mo(m) )  THEN
             surf_lsm%z0q(m) = 0.1_wp * surf_lsm%z_mo(m)
             flag_exceed_z0h   = .TRUE.
          ENDIF
...
#endif
    IF ( flag_exceed_z0 )  THEN
       WRITE( message_string, * ) 'z0 exceeds surface-layer height at sea surface(s) and ' //      &
                                  'is decreased appropriately'
...
#endif
    IF ( flag_exceed_z0h )  THEN
       WRITE( message_string, * ) 'z0h/q exceeds surface-layer height at sea surface(s) and ' //   &
                                  'is decreased appropriately'
       CALL message( 'land_surface_model_mod', 'PA0508', 0, 0, 0, 6, 0 )
    ENDIF
